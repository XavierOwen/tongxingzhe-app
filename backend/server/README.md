...
这个模块提供可信 session context、个人推广项目选择／创建、规范区域解析、同步 command 和 change feed。所有端点都先验证 Supabase access token，再取得内部用户和允许访问的项目。同步协议处理已提交接触、追加更正、带原因作废、未获回应尝试和账号私有草稿；设备专用草稿不会离开本机。
