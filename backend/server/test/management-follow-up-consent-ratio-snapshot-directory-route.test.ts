...
const endpointPath =
  `/v1/projects/${projectId}/management-follow-up-consent-ratio-report-snapshots`;
const identity = {issuer: "issuer", subject: "consent-directory-viewer"};
const accessEventId = "6edd0000-0000-4000-8000-000000000001";
const snapshotId = "6ea00000-0000-4000-8000-000000000001";

test("consent-ratio directory route awaits only its dedicated store", async () => {
  ...
  const server = createBackendServer({
    ...
    contextStore: {
      loadOrCreate: async () => {
        contextCalls += 1;
        throw new Error("SessionContext must not authorize consent directory");
      },
    },
    ...
    managementFollowUpConsentRatioReportSnapshotStore: {
      read: async () => {
        detailStoreCalls += 1;
        throw new Error("detail reader must not run");
      },
    },
    managementFollowUpConsentRatioSnapshotDirectoryStore: {
      list: async (receivedIdentity, receivedProjectId) => {
        ...
        return {
          accessContractId:
            "authorized_follow_up_consent_ratio_management_report_snapshot_directory_v1" as const,
          accessEventId,
          projectId,
          snapshots: [{
            snapshotId,
            reportId: "contact_target_follow_up_consent_ratio_two_periods" as const,
...
test("consent-ratio directory route returns an authorized empty directory", async () => {
  const server = createBackendServer({
    identityVerifier: {verify: async () => identity},
    contextStore: {
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}${endpointPath}`, {
    headers: {authorization: "Bearer token"},
  });
...
test("consent-ratio directory route authenticates before path, query, body, or store validation", async () => {
  let verifyCalls = 0;
  const server = createBackendServer({
    identityVerifier: {
      verify: async (token) => {
  ...
  const unauthenticated = await fetch(
    `http://127.0.0.1:${address.port}/v1/projects/not-a-uuid` +
      "/management-follow-up-consent-ratio-report-snapshots?filter=secret",
  );
  ...
  const verifierText = await verifierUnavailable.text();
  assert.deepEqual(JSON.parse(verifierText), {
    error: {
      code:
        "management_follow_up_consent_ratio_snapshot_directory_unavailable",
    },
  });
  assert.doesNotMatch(verifierText, /secret|verifier detail/);
...
test("consent-ratio directory route maps typed and unknown store errors without leaking details", async () => {
  ...
  const server = createBackendServer({
    ...
    managementFollowUpConsentRatioSnapshotDirectoryStore: {
      list: async () => {
        ...
        throw new Error("SQLSTATE 42501 secret consent directory detail");
      },
    },
  });
  const address = await listen(server);
  ...
  assert.doesNotMatch(body, /42501|SQLSTATE|consent directory detail/);
});
