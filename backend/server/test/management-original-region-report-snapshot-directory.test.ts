...
import {
  ...
  type ManagementOriginalRegionReportSnapshotDirectoryRead,
...
const firstSnapshotId = "6ea00000-0000-4000-8000-000000000001";
const secondSnapshotId = "6ea00000-0000-4000-8000-000000000002";
...
function storedDirectory(
  snapshots: readonly ManagementOriginalRegionReportSnapshotDirectoryRead["snapshots"][number][] = [{
...
test("authorized original-region directory returns only six bounded fields", async () => {
  ...
  assert.doesNotMatch(
    ...
    /protected_report|cells|source_tree|contributor|membership|capability|subject/,
  );
});

test("authorized original-region directory accepts an empty result", async () => {
  const result = await listManagementOriginalRegionReportSnapshotDirectory(
    request(),
    {
      identityVerifier: {verify: async () => identity},
  ...
  assert.deepEqual(result, {
    status: 200,
    body: {access_event_id: accessEventId, project_id: projectId, snapshots: []},
  });
});

test("original-region directory adapter uses one fixed 0071 bridge query", async () => {
...
test("original-region directory parser rejects malformed, extra, or unstable documents", async () => {
  const valid = directoryDocument();
  const invalidDirectories: readonly unknown[] = [
    {...valid, internal_user_id: "secret"},
    {...valid, access_contract_id: "authorized_interest_management_report_snapshot_directory_v1"},
    {...valid, project_id: "6eb30000-0000-4000-8000-000000000002"},
    directoryDocument(Array.from({length: 21}, (_, index) => ({
      ...firstItem,
      snapshot_id: `6ea00000-0000-4000-8000-${String(index).padStart(12, "0")}`,
    }))),
    ...
    directoryDocument([{...firstItem, source_tree_version: "secret"}]),
  ];
...
test("original-region directory authentication precedes request and store validation", async () => {
...
test("original-region directory maps request, authorization, and database failures", async () => {
  const baseRequest = request();
  const identityVerifier = {verify: async () => identity};

  assert.deepEqual(
    await listManagementOriginalRegionReportSnapshotDirectory(
  ...
  for (const value of [
    {
      error: new ManagementOriginalRegionReportSnapshotDirectoryStoreError(
        "forbidden",
      ),
      status: 403,
      code: "management_original_region_report_snapshot_directory_forbidden",
    },
    {
