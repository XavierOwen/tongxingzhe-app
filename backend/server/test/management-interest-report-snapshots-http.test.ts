...
function completed(): ManagementInterestReportSnapshotRead {
  return {
    status: "completed",
    accessEventId,
...
test("interest snapshot HTTP handler authenticates before request validation or storage", async () => {
  let verifyCalls = 0;
  let storeCalls = 0;
  const dependencies = {
    identityVerifier: {
  ...
  assert.deepEqual(
    await readManagementInterestReportSnapshot(
...
test("interest snapshot HTTP handler rejects invalid IDs, query, and body without storage", async () => {
  let storeCalls = 0;
  const dependencies = {
...
test("interest snapshot HTTP handler waits for the adapter and preserves the fixed report response", async () => {
  let finishRead: (() => void) | undefined;
  const readGate = new Promise<void>((resolve) => {finishRead = resolve;});
  const resultPromise = readManagementInterestReportSnapshot(
    request(),
    {
      identityVerifier: {verify: async () => identity},
      snapshotStore: {
...
test("interest snapshot HTTP handler maps typed outcomes and hides unknown database errors", async () => {
  const outcomes: ReadonlyArray<{
    readonly stored: ManagementInterestReportSnapshotRead;
  ...
  const forbidden = await readManagementInterestReportSnapshot(
    request(),
    {
      identityVerifier: {verify: async () => identity},
      snapshotStore: {
  ...
  assert.deepEqual(forbidden, {
    status: 403,
    body: {error: {code: "management_interest_report_snapshot_forbidden"}},
  });

  const unknown = await readManagementInterestReportSnapshot(
    request(),
    {
      identityVerifier: {verify: async () => identity},
      snapshotStore: {
...
test("interest snapshot HTTP handler maps verifier and absent adapter failures stably", async () => {
  const invalidIdentity = await readManagementInterestReportSnapshot(
    request(),
    {
  ...
  const verifierUnavailable = await readManagementInterestReportSnapshot(
    request(),
    {
  ...
  const missingStore = await readManagementInterestReportSnapshot(
    request(),
    {identityVerifier: {verify: async () => identity}},
  );
