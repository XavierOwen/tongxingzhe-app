-- 0072_management_original_region_report_snapshot_replacements.sql
--
-- Slice 6BN records an append-only, value-free direct replacement relation
-- between two already-approved 6BG original-region snapshots.  It does not
...
-- the fixed original-region family to the closed lifecycle role; the role
-- keeps its separate 6BE channel policy and gains no current-city, interest,
-- or legacy snapshot scope here.
CREATE POLICY
...
CREATE TABLE app_private.management_original_region_report_snapshot_replacements (
...
CREATE INDEX
  management_original_region_snapshot_replacements_project_idx
ON app_private.management_original_region_report_snapshot_replacements (
  project_id,
  release_lineage_id,
...
CREATE TRIGGER
  management_original_region_snapshot_replacements_immutable
BEFORE UPDATE OR DELETE
ON app_private.management_original_region_report_snapshot_replacements
FOR EACH ROW
EXECUTE FUNCTION app_private.reject_management_report_history_mutation();

-- The existing original-region release writer owns the 0068 attempt ledger.
...
CREATE FUNCTION
  app_private.original_region_snapshot_replacement_provenance_v1(
...
AS $function$
  ...
  JOIN app_private.management_original_region_report_release_attempts AS attempt
...
CREATE FUNCTION
  app_private.management_original_region_snapshot_has_trusted_provenance_v1(
    requested_snapshot_id uuid,
    requested_project_id uuid
  )
RETURNS boolean
...
SET search_path = pg_catalog, app_private
AS $function$
  /* The bridge verifies management_original_region_report_release_attempts,
     management_report_release_request_claims, and the original_region_management_report_snapshot_release
     claim; this function also rechecks source_tree_context. */
  SELECT EXISTS (
    SELECT 1
    FROM app_private.management_report_snapshots AS snapshot
    WHERE snapshot.snapshot_id = requested_snapshot_id
      ...
      AND snapshot.query_fingerprint =
        'management-report:contact_sessions_by_original_region_two_periods:v1'
      AND app_private.original_region_snapshot_replacement_provenance_v1(
...
BEGIN
  IF NEW.capability_id IS DISTINCT FROM 'release_management_reports' THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region replacement capability';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region replacement authorization lineage';
  END IF;
  ...
  IF superseded_snapshot.report_id <>
    ...
    OR superseded_snapshot.protected_report->'source_tree_context' IS DISTINCT FROM
      replacement_snapshot.protected_report->'source_tree_context'
    OR (
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'original-region replacement lineage is inconsistent';
  END IF;
  ...
  IF superseded_provenance IS NULL OR replacement_provenance IS NULL
    ...
    OR superseded_provenance->>'source_tree_version' <>
      replacement_provenance->>'source_tree_version'
    OR superseded_provenance->>'source_content_fingerprint' <>
      replacement_provenance->>'source_content_fingerprint'
    ...
    OR replacement_provenance->>'reporting_time_zone' <>
      replacement_snapshot.reporting_time_zone
    OR superseded_provenance->>'source_tree_version' IS DISTINCT FROM
      superseded_snapshot.protected_report->'source_tree_context'->>
        'source_tree_version'
    OR replacement_provenance->>'source_tree_version' IS DISTINCT FROM
      replacement_snapshot.protected_report->'source_tree_context'->>
        'source_tree_version'
    OR superseded_provenance->>'source_content_fingerprint' IS DISTINCT FROM
      superseded_snapshot.protected_report->'source_tree_context'->>
        'source_content_fingerprint'
    OR replacement_provenance->>'source_content_fingerprint' IS DISTINCT FROM
      replacement_snapshot.protected_report->'source_tree_context'->>
        'source_content_fingerprint'
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'original-region replacement provenance is inconsistent';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'original-region replacement provenance is untrusted';
  END IF;
  ...
  IF NEW.result_document <> expected_result_document THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region replacement result document';
  END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region replacement request';
  END IF;
  ...
  IF FOUND THEN
    ...
    THEN
      RAISE EXCEPTION USING
        ERRCODE = '22023',
        MESSAGE = 'original-region replacement idempotency conflict';
    END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'original-region replacement snapshot is unavailable';
  END IF;
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'management-original-region-snapshot-replacement-lineage:'
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original-region snapshot is no longer an active head';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'replacement snapshot already belongs to an original-region replacement chain';
  END IF;
  ...
  INSERT INTO app_private.management_original_region_report_snapshot_replacements (
...
BEGIN
  IF requested_project_id IS NULL OR requested_snapshot_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region snapshot lifecycle request';
  END IF;
...
COMMENT ON TABLE
  app_private.management_original_region_report_snapshot_replacements
IS 'Immutable value-free direct replacement links between existing trusted original-region report snapshots.';

COMMENT ON TABLE app_private.management_report_release_request_claims
IS 'Value-free request UUID ownership across management report release families and original-region snapshot replacement.';

COMMENT ON FUNCTION
  app_private.declare_management_original_region_snapshot_replacement_v1(
    uuid, uuid, uuid, uuid, uuid, text
  )
IS 'Reauthorizes and appends one idempotent original-region snapshot replacement without mutating or returning either protected report.';

COMMENT ON FUNCTION
  app_private.read_management_original_region_report_snapshot_lifecycle_v1(
    uuid, uuid
  )
IS 'Returns value-free active, superseded, or not-found lifecycle state for one trusted original-region snapshot.';
