-- Require current authority and legal intervals at the immutable write time.
...
BEGIN
  ...
  IF NOT FOUND
    OR target_membership_row.organization_workspace_id IS DISTINCT FROM
      requested_organization_workspace_id
    OR target_membership_row.inactive_from_utc IS NOT NULL
    OR NOT tstzrange(
  ...
  -- Only a new grant/close needs legal intervals at the immutable write time.
  -- Preserve the already-owner conflict before rejecting an unwritable close.
  IF actor_owner_assignment_row.active_from_utc >= effective_time
    OR target_membership_row.active_from_utc > effective_time
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'organization owner transfer forbidden';
