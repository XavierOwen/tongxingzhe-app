-- 0071_authorized_management_original_region_report_snapshot_directory.sql
--
-- Slice 6BK exposes a bounded metadata-only directory for the 0068
-- original-region snapshot lineage.  The directory has its own immutable,
-- value-free audit and rechecks the complete original-region provenance
-- tuple.  It never selects a latest/current snapshot and never returns a
-- protected report.

CREATE TABLE app_private.management_original_region_snapshot_directory_access_events (
...
CREATE INDEX management_original_region_snapshot_directory_idx
ON app_private.management_original_region_snapshot_directory_access_events (
  project_id,
...
CREATE TRIGGER management_original_region_snapshot_directory_access_immutable
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region snapshot directory authorization';
  END IF;
...
CREATE FUNCTION
  app_private.list_authorized_management_original_region_report_snapshots_v1(
...
BEGIN
  IF requested_app_user_id IS NULL OR requested_project_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original-region snapshot directory request';
  END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid runtime original-region snapshot directory request';
  END IF;
  ...
  IF resolved_app_user_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'original-region snapshot directory access forbidden';
  END IF;

  RETURN app_private.list_authorized_management_original_region_report_snapshots_v1(
...
BEGIN
  ...
  EXECUTE format(
    'ALTER TABLE app_private.management_original_region_snapshot_directory_access_events OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.validate_original_region_snapshot_directory_access_v1() OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.list_authorized_management_original_region_report_snapshots_v1(uuid,uuid) OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_data.list_authorized_management_original_region_report_snapshots_v1(text,text,uuid) OWNER TO %I',
    trusted_owner
  );
END
$owner$;
...
COMMENT ON TABLE
  app_private.management_original_region_snapshot_directory_access_events
IS 'Immutable minimal audit for one authorized original-region snapshot directory read; it stores no snapshot identifiers, report metadata, source tuple, cells or PII.';
...
IS 'Reauthorizes and lists at most twenty trusted original-region snapshot metadata records while committing one value-free directory audit.';
...
IS 'Maps one exact existing active identity and performs one bounded authorized original-region snapshot directory read without exposing app_private.';
