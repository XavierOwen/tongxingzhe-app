-- 0064_runtime_authorized_management_interest_report_snapshot_read.sql
--
-- Expose one narrow runtime bridge for the private 0063 interest snapshot
-- read. Backend verifies the external token before passing issuer/subject;
-- this function maps only an existing active identity and never creates
-- accounts, personal context or authorization state.

CREATE FUNCTION app_data.read_authorized_management_interest_report_snapshot_v1(
  trusted_issuer text,
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid runtime interest management report snapshot request';
  END IF;

  -- Match the stored identity exactly. btrim above is only an input-length
  -- guard; it must not turn a different issuer or subject into a match.
  ...
  IF resolved_app_user_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'interest management report snapshot access forbidden';
  END IF;

  -- 0063 owns authorization, provenance validation, locking, audit and the
  -- completed/not_found/untrusted_provenance response contract.
  RETURN app_private.read_authorized_management_interest_report_snapshot_v1(
...
BEGIN
  ...
  WHERE function_row.oid =
    'app_private.read_authorized_management_interest_report_snapshot_v1(uuid,uuid,uuid)'::regprocedure;

  EXECUTE format(
    'ALTER FUNCTION app_data.read_authorized_management_interest_report_snapshot_v1(text,text,uuid,uuid) OWNER TO %I',
    trusted_owner
  );
END
$owner$;
...
IS 'Maps one Backend-verified existing active identity and performs the fixed 0063 authorized interest snapshot read without exposing app_private.';
