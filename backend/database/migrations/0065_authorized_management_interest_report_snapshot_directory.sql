-- 0065_authorized_management_interest_report_snapshot_directory.sql
--
-- Slice 6BA exposes a bounded interest snapshot directory.  It has its
-- own directory audit and provenance filter; the generic channel and
-- current-city directories must never become implicit readers for the 0062
-- interest lineage.

CREATE TABLE app_private.management_interest_report_snapshot_directory_access_events (
...
CREATE INDEX management_interest_snapshot_directory_access_project_idx
ON app_private.management_interest_report_snapshot_directory_access_events (
  project_id,
...
CREATE TRIGGER management_interest_snapshot_directory_access_immutable
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot directory authorization';
  END IF;
...
CREATE TRIGGER management_interest_snapshot_directory_access_validate
BEFORE INSERT
ON app_private.management_interest_report_snapshot_directory_access_events
FOR EACH ROW
EXECUTE FUNCTION
...
BEGIN
  IF requested_app_user_id IS NULL OR requested_project_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot directory request';
  END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid runtime interest snapshot directory request';
  END IF;
  ...
  IF resolved_app_user_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'interest snapshot directory access forbidden';
  END IF;

  RETURN app_private.list_authorized_management_interest_report_snapshots_v1(
...
BEGIN
  ...
  EXECUTE format(
    'ALTER TABLE app_private.management_interest_report_snapshot_directory_access_events OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.validate_management_interest_snapshot_directory_access_v1() OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.list_authorized_management_interest_report_snapshots_v1(uuid,uuid) OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_data.list_authorized_management_interest_report_snapshots_v1(text,text,uuid) OWNER TO %I',
    trusted_owner
  );
END
$owner$;
...
COMMENT ON TABLE
  app_private.management_interest_report_snapshot_directory_access_events
IS 'Immutable minimal audit for one authorized interest snapshot directory read; it stores no snapshot identifiers, report metadata or cells.';
...
IS 'Reauthorizes and lists at most twenty trusted interest snapshot metadata records while committing one value-free directory audit.';
...
IS 'Maps one exact existing active identity and performs one bounded authorized interest snapshot directory read without exposing app_private.';
