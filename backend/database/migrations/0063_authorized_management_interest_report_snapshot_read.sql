-- 0063_authorized_management_interest_report_snapshot_read.sql
--
-- Slice 6AX adds a private, authorized read for one 6AW interest snapshot.
-- It deliberately does not reuse the generic channel reader or the
-- current-city reader: the protected document, release attempt and shared
-- claim family must all belong to the fixed interest contract.

CREATE TABLE app_private.management_interest_report_snapshot_access_events (
  ...
  interest_release_request_id uuid NULL
    REFERENCES app_private.management_interest_report_release_attempts (
      release_request_id
    ),
  report_id text NULL,
  ...
  CHECK (
    (
      ...
      AND interest_release_request_id IS NOT NULL
      AND report_id =
    ...
    OR (
      ...
      AND interest_release_request_id IS NULL
...
CREATE INDEX management_interest_snapshot_access_events_project_idx
ON app_private.management_interest_report_snapshot_access_events (
  project_id,
...
DECLARE
  ...
  has_interest_provenance boolean := false;
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot access authorization';
  END IF;

  IF NEW.capability_id IS DISTINCT FROM 'view_anonymous_analytics' THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot access capability';
  END IF;
  ...
  IF NEW.result_status = 'not_found' THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION USING
        ERRCODE = '22023',
        MESSAGE = 'invalid missing interest snapshot access';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot access snapshot identity';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot access metadata';
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE attempt.release_request_id = NEW.interest_release_request_id
  ...
  ) INTO has_interest_provenance;

  IF NOT has_interest_provenance THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid trusted interest snapshot access provenance';
  END IF;
...
CREATE FUNCTION app_private.read_authorized_management_interest_report_snapshot_v1(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid authorized interest snapshot request';
  END IF;
  ...
  ELSE
    ...
    IF FOUND
      AND stored_snapshot.release_request_id =
        interest_attempt.release_request_id
      AND stored_snapshot.report_id = interest_attempt.report_id
      AND stored_snapshot.report_version = interest_attempt.report_version
      AND stored_snapshot.query_fingerprint =
        interest_attempt.query_fingerprint
      AND stored_snapshot.release_lineage_id =
        interest_attempt.release_lineage_id
      AND stored_snapshot.data_cutoff_utc = interest_attempt.data_cutoff_utc
      AND stored_snapshot.source_change_sequence =
        interest_attempt.source_change_sequence
      AND stored_snapshot.previous_snapshot_id IS NOT DISTINCT FROM
        interest_attempt.compared_snapshot_id
    THEN
      BEGIN
        PERFORM app_private.validate_management_interest_report_document_v1(
          stored_snapshot.protected_report
  ...
  INSERT INTO app_private.management_interest_report_snapshot_access_events (
    ...
    CASE
      WHEN result_status_value = 'completed'
        THEN interest_attempt.release_request_id
      ELSE NULL
    END,
    CASE
      WHEN result_status_value = 'not_found' THEN NULL
    ...
    CASE
      WHEN result_status_value = 'not_found' THEN NULL
      ELSE stored_snapshot.source_change_sequence
    END,
    authorization_reference_at_utc_value,
    result_status_value,
    reason_code_value
...
BEGIN
  ...
  EXECUTE format(
    'ALTER TABLE app_private.management_interest_report_snapshot_access_events OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.validate_management_interest_snapshot_access_insert_v1() OWNER TO %I',
    trusted_owner
  );
  EXECUTE format(
    'ALTER FUNCTION app_private.read_authorized_management_interest_report_snapshot_v1(uuid,uuid,uuid) OWNER TO %I',
    trusted_owner
  );
END
$owner$;

CREATE TRIGGER management_interest_snapshot_access_events_immutable
BEFORE UPDATE OR DELETE
ON app_private.management_interest_report_snapshot_access_events
FOR EACH ROW
EXECUTE FUNCTION app_private.reject_management_report_history_mutation();

CREATE TRIGGER management_interest_snapshot_access_events_validate
BEFORE INSERT
ON app_private.management_interest_report_snapshot_access_events
FOR EACH ROW
EXECUTE FUNCTION
...
COMMENT ON TABLE
  app_private.management_interest_report_snapshot_access_events
IS 'Immutable private audit for authorized interest snapshot reads; it stores no report cells, contributors, contact identities or PII.';
...
IS 'Reauthorizes one fixed interest snapshot read, accepts only 6AW interest provenance, validates the ten-cell document, and records a value-free audit.';
