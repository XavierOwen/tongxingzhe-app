-- 0082_management_interest_report_snapshot_replacements.sql
--
-- Slice 6CD records an append-only, value-free direct replacement relation
-- between two already-approved 6AW interest snapshots. It does not
...
-- the fixed interest family to the closed lifecycle role; the role
...
CREATE TABLE app_private.management_interest_report_snapshot_replacements (
...
CREATE INDEX
  management_interest_snapshot_replacements_project_idx
ON app_private.management_interest_report_snapshot_replacements (
  project_id,
  release_lineage_id,
...
CREATE TRIGGER
  management_interest_snapshot_replacements_immutable
BEFORE UPDATE OR DELETE
ON app_private.management_interest_report_snapshot_replacements
FOR EACH ROW
EXECUTE FUNCTION app_private.reject_management_report_history_mutation();

-- The existing interest release writer owns the 0062 attempt ledger.
...
CREATE FUNCTION
  app_private.interest_snapshot_replacement_provenance_v1(
...
AS $function$
  ...
  JOIN app_private.management_interest_report_release_attempts AS attempt
...
CREATE FUNCTION
  app_private.management_interest_snapshot_has_trusted_provenance_v1(
    requested_snapshot_id uuid,
    requested_project_id uuid
  )
RETURNS boolean
...
SET search_path = pg_catalog, app_private
AS $function$
  /* Split the trust check across the two closed owners: the interest
  ...
  SELECT EXISTS (
    ...
    WHERE snapshot.snapshot_id = requested_snapshot_id
      ...
      AND app_private.interest_snapshot_replacement_provenance_v1(
...
BEGIN
  IF NEW.capability_id IS DISTINCT FROM 'release_management_reports' THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest replacement capability';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest replacement authorization lineage';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'interest replacement lineage is inconsistent';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'interest replacement provenance is inconsistent';
  END IF;

  IF NOT app_private.management_interest_snapshot_has_trusted_provenance_v1(
      superseded_snapshot.snapshot_id,
      NEW.project_id
    )
    OR NOT app_private.management_interest_snapshot_has_trusted_provenance_v1(
      replacement_snapshot.snapshot_id,
      NEW.project_id
    )
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'interest replacement provenance is untrusted';
  END IF;
  ...
  IF NEW.result_document <> expected_result_document THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest replacement result document';
  END IF;
...
CREATE FUNCTION
  app_private.declare_management_interest_snapshot_replacement_v1(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest replacement request';
  END IF;
  ...
  IF FOUND THEN
    ...
    THEN
      RAISE EXCEPTION USING
        ERRCODE = '22023',
        MESSAGE = 'interest replacement idempotency conflict';
    END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'interest replacement snapshot is unavailable';
  END IF;
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'management-interest-snapshot-replacement-lineage:'
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'interest snapshot is no longer an active head';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'replacement snapshot already belongs to an interest replacement chain';
  END IF;
  ...
  INSERT INTO app_private.management_interest_report_snapshot_replacements (
...
BEGIN
  IF requested_project_id IS NULL OR requested_snapshot_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid interest snapshot lifecycle request';
  END IF;
...
COMMENT ON TABLE
  app_private.management_interest_report_snapshot_replacements
IS 'Immutable value-free direct replacement links between existing trusted interest report snapshots.';

COMMENT ON TABLE app_private.management_report_release_request_claims
IS 'Value-free request UUID ownership across management report release families and interest snapshot replacement.';

COMMENT ON FUNCTION
  app_private.declare_management_interest_snapshot_replacement_v1(
    uuid, uuid, uuid, uuid, uuid, text
  )
IS 'Reauthorizes and appends one idempotent interest snapshot replacement without mutating or returning either protected report.';

COMMENT ON FUNCTION
  app_private.read_management_interest_report_snapshot_lifecycle_v1(
    uuid, uuid
  )
IS 'Returns value-free active, superseded, or not-found lifecycle state for one trusted interest snapshot.';
