-- 0044_personal_target_response_distribution.sql
--
-- 为个人对象当次反应提供一个窄的有序分布入口。统计单位是当前有效
-- contact revision 中的接触对象关联；只有 response_level 非 NULL 的
-- 关联进入五档分母，NULL 关联作为 unanswered_count 单独保留。

CREATE FUNCTION app_data.read_personal_target_response_distribution(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid personal target response distribution request';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'personal target response distribution scope forbidden';
  END IF;
  ...
  GROUP BY levels.response_level,
           aggregate.denominator,
           aggregate.unanswered_count
  ORDER BY levels.response_level;
END
$function$;

COMMENT ON FUNCTION app_data.read_personal_target_response_distribution(
  uuid, uuid, uuid, timestamptz, timestamptz
) IS
  'Backend-only personal target response distribution over a UTC half-open interval; rows 0..4 are ordered by response_level, denominator counts only non-NULL responses from current active contact revisions, and unanswered_count reports current links whose response_level is NULL.';

REVOKE ALL
  ON FUNCTION app_data.read_personal_target_response_distribution(
    uuid, uuid, uuid, timestamptz, timestamptz
  )
  FROM PUBLIC;
