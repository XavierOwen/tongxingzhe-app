-- 0066_management_original_region_report.sql
--
-- Slice 6BD registers and executes one private original-region report.  The
-- report deliberately uses the city ancestor of each exact original source
-- tuple.  It never reads current selection, re-resolves coordinates, or
-- invokes a canonical-region version mapping.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE rolname = 'tongxingzhe_management_original_region_report_reader'
  ) THEN
    CREATE ROLE tongxingzhe_management_original_region_report_reader
      NOLOGIN
      NOSUPERUSER
      NOCREATEDB
      NOCREATEROLE
...
ALTER ROLE tongxingzhe_management_original_region_report_reader
  NOLOGIN
  NOSUPERUSER
  NOCREATEDB
  NOCREATEROLE
...
-- Hard-code the original view at this seam.  The report reader can therefore
-- never supply a target tree that would silently turn this report into a
-- current or mapped attribution.
CREATE FUNCTION
  app_private.resolve_management_original_region_attribution_v1(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid management original region report request';
  END IF;
...
CREATE FUNCTION
  app_private.protect_management_original_region_contact_session_grid_v1(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original region report city grid';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid original region report contributions';
  END IF;

  -- Keep the private result within the exact integer range used by JSON
  -- clients.  Numeric aggregation avoids bigint overflow before this check.
  IF EXISTS (
    WITH input_rows AS (
      SELECT
        item->>'period_key' AS input_period_key,
        btrim(item->>'city_id') AS input_city_id,
    ...
    aggregate_rows AS (
      ...
      SELECT
        ...
        sum(input_unit_count) AS aggregate_count
      FROM input_rows
      GROUP BY input_period_key, input_city_id
    )
    SELECT 1
    FROM aggregate_rows
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'original region report aggregate is too large';
  END IF;
  ...
  input_rows AS (
    SELECT
      ...
      (contribution.item->>'unit_count')::numeric AS input_unit_count
...
BEGIN
  ...
  IF requested_project_id IS NULL OR NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid management original region report project';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report period definition is inconsistent';
  END IF;
  ...
  evidence_status AS (
    SELECT coalesce(bool_or(
      ...
      OR attribution.attribution_document->>'result_status'
        IS DISTINCT FROM 'attributed'
      OR attribution.attribution_document->>'reason_code'
        IS DISTINCT FROM 'original_exact_source'
      OR attribution.attribution_document->>'region_id' IS NULL
      OR attribution.attribution_document->>'tree_version' IS NULL
      OR attribution.attribution_document->>'content_fingerprint'
        !~ '^[0-9a-f]{64}$'
      OR attribution.attribution_document ?| ARRAY[
        'source_id', 'contact_id', 'revision_number', 'contributor_key',
        'canonical_name', 'place_name', 'latitude', 'longitude', 'geometry'
      ]
  ...
  tree_tuples AS (
    SELECT DISTINCT
      attribution.attribution_document->>'tree_version'
        AS source_tree_version,
      attribution.attribution_document->>'content_fingerprint'
        AS source_content_fingerprint
  ...
  region_ancestors AS (
    SELECT
      ...
      attribution.contributor_key,
      attribution.period_key,
      attribution.attribution_document->>'tree_version' AS tree_version,
      node.region_id,
      node.parent_region_id,
      node.kind
    FROM attributed AS attribution
    JOIN app_data.canonical_region_versions AS node
      ON node.tree_version = attribution.attribution_document->>'tree_version'
  ...
  lineage_status AS (
    SELECT
      ...
      EXISTS (
        SELECT 1
        FROM region_ancestors AS ancestor
        WHERE ancestor.parent_region_id IS NOT NULL
          AND NOT EXISTS (
            SELECT 1
            FROM app_data.canonical_region_versions AS parent
            WHERE parent.tree_version = ancestor.tree_version
              AND parent.region_id = ancestor.parent_region_id
          )
      ) AS has_missing_parent
  ...
  SELECT
    (SELECT count(*) FROM candidate_contacts),
    ...
    (SELECT lineage.has_invalid_city_ancestor FROM lineage_status AS lineage),
    (SELECT lineage.has_missing_parent FROM lineage_status AS lineage),
    (SELECT count(*)::integer FROM tree_tuples),
    (SELECT min(tree.source_tree_version) FROM tree_tuples AS tree),
    (SELECT min(tree.source_content_fingerprint) FROM tree_tuples AS tree),
    coalesce(
      (SELECT jsonb_agg(
        jsonb_build_object(
  ...
  IF missing_source OR invalid_attribution THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report attribution evidence is not reportable';
  END IF;

  IF invalid_city_ancestor OR missing_parent THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report city lineage is inconsistent';
  END IF;

  IF source_tree_count > 1 THEN
    source_tree_context = jsonb_build_object(
      'source_tree_context_contract_id',
        'management-original-region-source-tree:v1',
      'result_status', 'unavailable',
      'reason_code', 'source_tree_mixed'
    );
    RETURN jsonb_build_object(
      ...
      'source_tree_context', source_tree_context,
      'result_status', 'unavailable',
      'reason_code', 'source_tree_mixed'
    );
  END IF;

  IF candidate_count = 0 OR source_tree_count = 0 THEN
    source_tree_context = jsonb_build_object(
      'source_tree_context_contract_id',
        'management-original-region-source-tree:v1',
      'result_status', 'unavailable',
      'reason_code', 'source_tree_unavailable'
    );
    RETURN jsonb_build_object(
      'report_id', canonical_request->'report_id',
      'report_version', canonical_request->'report_version',
  ...
  -- 6AL validates each source tuple.  Reconfirm the one selected tuple at
  -- the report boundary so a future caller cannot turn a stale source tuple
  -- into a report merely by changing the composition around this function.
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report source tree release is unavailable';
  END IF;
  ...
  SELECT
    ...
    count(*)::integer
  INTO city_ids_document, source_city_count
  FROM app_data.canonical_region_versions AS city
  WHERE city.tree_version = source_tree_version
    AND city.kind = 'city';

  IF source_city_count = 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report source tree has no cities';
  END IF;

  -- Nested city nodes make a source's city dimension ambiguous.  A missing
  -- parent was checked above for every source lineage; this check prevents a
  -- malformed full city grid from being emitted as if it were canonical.
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'original region report source tree has nested cities';
  END IF;
...
GRANT USAGE ON SCHEMA app_data, app_private
  TO tongxingzhe_management_original_region_report_reader;
...
REVOKE ALL PRIVILEGES ON TABLE
  ...
  FROM tongxingzhe_management_original_region_report_reader;
...
GRANT SELECT (region_id, tree_version, parent_region_id, kind)
  ...
  TO tongxingzhe_management_original_region_report_reader;
GRANT SELECT (tree_version, lifecycle_state, content_fingerprint)
...
-- The migration identity can run structural fixtures but does not retain the
-- dedicated reader role as a member after this migration transaction.
GRANT EXECUTE ON FUNCTION
  app_private.canonicalize_management_original_region_report_request_v1(
    jsonb
  ),
...
COMMENT ON FUNCTION
  app_private.resolve_management_original_region_attribution_v1(uuid)
IS 'Hard-codes the 6AL original attribution view and never accepts a target tree or mapping request.';

COMMENT ON FUNCTION
  app_private.canonicalize_management_original_region_report_request_v1(
    jsonb
  )
IS 'Canonicalizes only contact_sessions_by_original_region_two_periods@1; it performs no authorization or execution.';

COMMENT ON FUNCTION
  app_private.protect_management_original_region_contact_session_grid_v1(
    jsonb, jsonb
  )
IS 'Applies k=10, three-contributor, half-contribution and deterministic complementary hiding to a complete two-period original-city grid.';

COMMENT ON FUNCTION
  app_private.execute_management_original_region_contact_session_report_v1(
    uuid, text, timestamp with time zone
  )
IS 'Builds a private original-region report from exact source provenance, requires one source tree, and returns no source, contact, contributor or location PII.';
