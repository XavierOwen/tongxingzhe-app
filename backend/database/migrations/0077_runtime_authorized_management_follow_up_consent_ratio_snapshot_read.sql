-- 0077_runtime_authorized_management_follow_up_consent_ratio_snapshot_read.sql
--
-- Slice 6BS exposes one narrow runtime bridge for the private 6BR
-- follow-up consent-ratio snapshot read. Backend verifies the external token
-- before passing its issuer and subject. The bridge maps only an existing
-- active identity and never exposes app_private to tongxingzhe_runtime.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid runtime follow-up consent-ratio snapshot request';
  END IF;
  ...
  IF resolved_app_user_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'follow-up consent-ratio snapshot access forbidden';
  END IF;

  -- 0076 owns authorization, provenance validation, locking, audit and the
...
BEGIN
  ...
  WHERE function_row.oid =
    'app_private.read_authorized_management_follow_up_consent_report_snapshot_v1(uuid,uuid,uuid)'::regprocedure;

  EXECUTE format(
    'ALTER FUNCTION app_data.read_authorized_management_follow_up_consent_report_snapshot_v1(text,text,uuid,uuid) OWNER TO %I',
    trusted_owner
  );
END
$owner$;
...
IS 'Maps one Backend-verified exact external identity and performs the fixed 0076 authorized follow-up consent-ratio snapshot read without exposing app_private.';
