-- 0060_authorized_management_current_city_report_snapshot_directory.sql
--
-- Slice 6AS exposes a bounded current-city snapshot directory.  It has its
-- own directory audit and provenance filter; the generic channel directory
-- must never become an implicit reader for the 0057 current-city lineage.

CREATE TABLE app_private.management_current_city_report_snapshot_directory_access_events (
...
CREATE INDEX management_current_city_snapshot_directory_access_project_idx
ON app_private.management_current_city_report_snapshot_directory_access_events (
  project_id,
...
CREATE TRIGGER management_current_city_snapshot_directory_access_immutable
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid current-city snapshot directory authorization';
  END IF;
...
CREATE TRIGGER management_current_city_snapshot_directory_access_validate
BEFORE INSERT
ON app_private.management_current_city_report_snapshot_directory_access_events
FOR EACH ROW
EXECUTE FUNCTION
...
BEGIN
  IF requested_app_user_id IS NULL OR requested_project_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid current-city snapshot directory request';
  END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid runtime current-city snapshot directory request';
  END IF;
  ...
  IF resolved_app_user_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'current-city snapshot directory access forbidden';
  END IF;

  RETURN app_private.list_authorized_management_current_city_report_snapshots_v1(
...
COMMENT ON TABLE
  app_private.management_current_city_report_snapshot_directory_access_events
IS 'Immutable minimal audit for one authorized current-city snapshot directory read; it stores no snapshot identifiers, report metadata or cells.';
...
IS 'Reauthorizes and lists at most twenty trusted current-city snapshot metadata records while committing one value-free directory audit.';
...
IS 'Maps one exact existing active identity and performs one bounded authorized current-city snapshot directory read without exposing app_private.';
