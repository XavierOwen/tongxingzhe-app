-- 0083_management_follow_up_consent_ratio_snapshot_replacements.sql
--
-- Slice 6CE records an append-only, value-free direct replacement relation
-- between two already-approved 6BQ follow-up consent ratio snapshots. It does not
...
-- the fixed follow-up consent ratio family to the closed lifecycle role; the role
...
CREATE TABLE app_private.management_follow_up_consent_ratio_report_snapshot_replacements (
...
CREATE INDEX
  management_follow_up_consent_ratio_snapshot_repl_project_idx
ON app_private.management_follow_up_consent_ratio_report_snapshot_replacements (
  project_id,
  release_lineage_id,
...
CREATE TRIGGER
  management_follow_up_consent_snapshot_replacements_immutable
BEFORE UPDATE OR DELETE
ON app_private.management_follow_up_consent_ratio_report_snapshot_replacements
FOR EACH ROW
EXECUTE FUNCTION app_private.reject_management_report_history_mutation();

-- The existing follow-up consent ratio release writer owns the 0075 attempt ledger.
...
CREATE FUNCTION
  app_private.follow_up_consent_ratio_snapshot_replacement_provenance_v1(
...
AS $function$
  ...
  JOIN app_private.management_follow_up_consent_report_release_attempts AS attempt
...
CREATE FUNCTION
  app_private.management_follow_up_consent_snapshot_has_trusted_provenance_v1(
    requested_snapshot_id uuid,
    requested_project_id uuid
  )
RETURNS boolean
...
SET search_path = pg_catalog, app_private
AS $function$
  /* Split the trust check across the two closed owners: the follow-up consent ratio
  ...
  SELECT EXISTS (
    ...
    WHERE snapshot.snapshot_id = requested_snapshot_id
      ...
      AND (
        snapshot.protected_report->'periods'->>'data_cutoff_utc'
      )::timestamptz =
        snapshot.data_cutoff_utc
      AND app_private.follow_up_consent_ratio_snapshot_replacement_provenance_v1(
...
BEGIN
  IF NEW.capability_id IS DISTINCT FROM 'release_management_reports' THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid follow-up consent ratio replacement capability';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid follow-up consent ratio replacement authorization lineage';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'follow-up consent ratio replacement lineage is inconsistent';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'follow-up consent ratio replacement provenance is inconsistent';
  END IF;

  IF NOT app_private.management_follow_up_consent_snapshot_has_trusted_provenance_v1(
      superseded_snapshot.snapshot_id,
      NEW.project_id
    )
    OR NOT app_private.management_follow_up_consent_snapshot_has_trusted_provenance_v1(
      replacement_snapshot.snapshot_id,
      NEW.project_id
    )
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'follow-up consent ratio replacement provenance is untrusted';
  END IF;
  ...
  IF NEW.result_document <> expected_result_document THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid follow-up consent ratio replacement result document';
  END IF;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid follow-up consent ratio replacement request';
  END IF;
  ...
  IF FOUND THEN
    ...
    THEN
      RAISE EXCEPTION USING
        ERRCODE = '22023',
        MESSAGE = 'follow-up consent ratio replacement idempotency conflict';
    END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'follow-up consent ratio replacement snapshot is unavailable';
  END IF;
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'management-follow-up-consent-ratio-snapshot-replacement-lineage:'
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'follow-up consent ratio snapshot is no longer an active head';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'replacement snapshot already belongs to a follow-up consent ratio replacement chain';
  END IF;
  ...
  INSERT INTO app_private.management_follow_up_consent_ratio_report_snapshot_replacements (
...
BEGIN
  IF requested_project_id IS NULL OR requested_snapshot_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'invalid follow-up consent ratio snapshot lifecycle request';
  END IF;
...
COMMENT ON TABLE
  app_private.management_follow_up_consent_ratio_report_snapshot_replacements
IS 'Immutable value-free direct replacement links between existing trusted follow-up consent ratio report snapshots.';

COMMENT ON TABLE app_private.management_report_release_request_claims
IS 'Value-free request UUID ownership across management report release families and follow-up consent ratio snapshot replacement.';

COMMENT ON FUNCTION
  app_private.declare_management_follow_up_consent_snapshot_replacement_v1(
    uuid, uuid, uuid, uuid, uuid, text
  )
IS 'Reauthorizes and appends one idempotent follow-up consent ratio snapshot replacement without mutating or returning either protected report.';

COMMENT ON FUNCTION
  app_private.read_management_follow_up_consent_snapshot_lifecycle_v1(
    uuid, uuid
  )
IS 'Returns value-free active, superseded, or not-found lifecycle state for one trusted follow-up consent ratio snapshot.';
