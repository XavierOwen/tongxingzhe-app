-- An organization committed by the shipped 0084 runtime writer before 0087.
...
VALUES
  ('00000000-0086-0000-0000-000000000601', 'active'),
  ('00000000-0086-0000-0000-000000000602', 'active');
...
VALUES
  (
    '00000000-0086-1000-0000-000000000601',
    'https://synthetic-directed-invitation-upgrade.example/auth/v1',
    'owner',
    '00000000-0086-0000-0000-000000000601'
  ),
  (
    '00000000-0086-1000-0000-000000000602',
    'https://synthetic-directed-invitation-upgrade.example/auth/v1',
    'target',
    '00000000-0086-0000-0000-000000000602'
  );

CREATE TEMP TABLE fixture_0086_creation_receipt (
...
GRANT ALL ON fixture_0086_creation_receipt TO tongxingzhe_runtime;

SET ROLE tongxingzhe_runtime;
INSERT INTO fixture_0086_creation_receipt
SELECT *
FROM app_data.create_organization_for_identity_v1(
  'https://synthetic-directed-invitation-upgrade.example/auth/v1',
  'owner',
  '00000000-0086-5000-0000-000000000601',
  '  0086 Directed invitation upgrade organization  '
);
RESET ROLE;

DO $legacy$
DECLARE
  receipt fixture_0086_creation_receipt%ROWTYPE;
  claim app_private.organization_creation_request_claims%ROWTYPE;
BEGIN
  SELECT * INTO STRICT receipt FROM fixture_0086_creation_receipt;
  SELECT * INTO STRICT claim
  FROM app_private.organization_creation_request_claims
  WHERE request_id = '00000000-0086-5000-0000-000000000601'::uuid;

  IF (SELECT count(*) FROM fixture_0086_creation_receipt) <> 1
    ...
    OR (SELECT count(*)
        ...
        WHERE creation_contract_id = receipt.creation_contract_id
          AND request_id = '00000000-0086-5000-0000-000000000601'::uuid
    ...
    OR (SELECT count(*)
        ...
        WHERE issuer =
          ...
          AND subject = 'target'
          AND app_user_id =
            '00000000-0086-0000-0000-000000000602'::uuid) <> 1
    OR EXISTS (
      SELECT 1
      FROM app_data.organization_memberships
      WHERE app_user_id =
  ...
  THEN
    RAISE EXCEPTION '0086 directed-invitation baseline drift';
  END IF;
END
$legacy$;

TABLE fixture_0086_creation_receipt;
