...
-- Structural and least-privilege checks for Slice 6CD.  The rollback fixture
-- proves the public declaration and lifecycle seams.  This check keeps the
-- interest relation independent from the 6BE channel ledger and makes
...
BEGIN
  ...
  IF replacements_table_oid IS NULL THEN
    RAISE EXCEPTION
      'interest snapshot replacement table is missing';
  END IF;
    ...
    OR NOT EXISTS (
      ...
      WHERE policy_row.polrelid = snapshots_table_oid
        ...
        AND pg_catalog.pg_get_expr(
          ...
          '((report_id = ''contact_sessions_by_interest_level_two_periods''::text) '
          || 'AND (release_lineage_id = '
          || '''management-interest-report:'
          || 'contact_sessions_by_interest_level_two_periods''::text))'
    )
  THEN
    RAISE EXCEPTION
      'interest snapshot replacement RLS scope is not exact';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'interest replacement owner must be a closed NOLOGIN role: %',
      table_owner;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'interest replacement table stores report values or source data';
  END IF;
  ...
  LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION
        'interest replacement column contract is incomplete: %',
  ...
  IF (
    ...
    RAISE EXCEPTION
      'interest replacement table must contain exactly seventeen audit columns';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'interest replacement request primary key is incomplete';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION
      'interest replacement snapshot uniqueness is incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'interest replacement snapshot foreign keys are incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'interest replacement table lacks the no-self-link invariant';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'interest replacement history is not immutable';
  END IF;
  ...
  IF function_name IS NULL THEN
    RAISE EXCEPTION 'interest replacement insert validator is missing';
  END IF;
  SELECT
  ...
  THEN
    RAISE EXCEPTION
      'interest replacement validator security/provenance contract is incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE trigger_row.tgrelid = replacements_table_oid
      AND trigger_row.tgname =
        'management_interest_snapshot_replacements_validate'
      AND NOT trigger_row.tgisinternal
      AND pg_get_triggerdef(trigger_row.oid) ILIKE
        '%validate_management_interest_snapshot_replacement_v1%'
  ) THEN
    RAISE EXCEPTION
      'interest replacement insert validator trigger is missing';
  END IF;
  ...
  IF trusted_function IS NULL THEN
    RAISE EXCEPTION
      'interest trusted provenance helper is missing';
  END IF;
  SELECT
    pg_get_userbyid(procedure_row.proowner),
    procedure_row.prosecdef,
  ...
  IF function_owner IS DISTINCT FROM table_owner
    ...
    OR function_definition NOT ILIKE '%interest_snapshot_replacement_provenance_v1%'
  ...
  THEN
    RAISE EXCEPTION
      'interest trusted provenance helper contract is incomplete';
  END IF;
  ...
  IF provenance_function IS NULL THEN
    RAISE EXCEPTION 'interest replacement provenance seam is missing';
  END IF;
  SELECT
    pg_get_userbyid(procedure_row.proowner),
    procedure_row.prosecdef,
  ...
  IF function_owner IS DISTINCT FROM
    ...
    OR function_definition NOT ILIKE '%management_interest_report_release_attempts%'
  ...
  THEN
    RAISE EXCEPTION
      'interest replacement provenance seam contract is incomplete';
  END IF;
  ...
  LOOP
    ...
    THEN
      RAISE EXCEPTION
        'interest replacement internal function is directly executable: %',
        function_name;
    END IF;
  END LOOP;
  ...
  LOOP
    IF function_name IS NULL THEN
      RAISE EXCEPTION
        'interest replacement public SQL seam is missing';
    END IF;
    ...
    THEN
      RAISE EXCEPTION
        'interest replacement function owner/security is incorrect: %',
        function_name;
    END IF;
    ...
    IF function_name::text LIKE 'app_private.declare_%' THEN
      IF function_volatility <> 'v'
        ...
        OR function_definition NOT ILIKE
          '%management_interest_snapshot_has_trusted_provenance_v1%'
        OR function_definition NOT ILIKE
          '%validate_management_interest_report_document_v1%'
      THEN
        RAISE EXCEPTION
          'interest replacement declaration lock/provenance contract is incomplete';
      END IF;
    ELSE
      ...
      THEN
        RAISE EXCEPTION
          'interest replacement lifecycle query is not value-free';
      END IF;
    END IF;
  END LOOP;
  ...
  THEN
    RAISE EXCEPTION
      'lifecycle owner can bypass the interest provenance bridge';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'interest replacement request claim contract is incomplete';
  END IF;

  -- The relation is private.  The interest reader and release writer
  ...
  LOOP
    IF EXISTS (
      ...
      RAISE EXCEPTION
        'interest replacement contract is directly executable by %',
        role_name;
    END IF;
  END LOOP;
  ...
  THEN
    RAISE EXCEPTION 'interest replacement contract is open to PUBLIC';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION
      'interest snapshot replacement migration was not recorded once';
  END IF;
END
$check$;
