...
-- Structural and least-privilege contract for the 6BS runtime bridge.  The
-- rollback fixture covers identity and delegated-read behavior; this check
-- keeps the bridge's owner, ACL, search path and private boundary explicit.
DO $check$
DECLARE
  bridge regprocedure := to_regprocedure(
    'app_data.read_authorized_management_follow_up_consent_report_snapshot_v1(text,text,uuid,uuid)'
  );
...
BEGIN
  IF bridge IS NULL OR private_read IS NULL THEN
    RAISE EXCEPTION
      'runtime follow-up consent-ratio snapshot bridge is incomplete';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'runtime follow-up consent-ratio snapshot bridge owner/security is incorrect';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'runtime follow-up consent-ratio snapshot bridge ACL is incorrect';
  END IF;
  ...
  FOREACH forbidden_role IN ARRAY ARRAY[
    ...
    IF has_function_privilege(forbidden_role, bridge, 'EXECUTE') THEN
      RAISE EXCEPTION
        'forbidden role can execute runtime follow-up consent-ratio snapshot bridge: %',
        forbidden_role;
    END IF;
  END LOOP;

  -- Runtime crosses the boundary only through this wrapper.  It must not
  -- receive app_private usage, private-reader execution, audit access, or
  -- identity-table SELECT privileges.
  ...
  THEN
    RAISE EXCEPTION
      'runtime received direct follow-up consent-ratio identity or private access';
  END IF;
  ...
  IF function_definition IS NULL
    ...
    OR function_definition NOT ILIKE
      '%RETURN app_private.read_authorized_management_follow_up_consent_report_snapshot_v1(%'
    OR (
      SELECT count(*)
  ...
  THEN
    RAISE EXCEPTION
      'runtime follow-up consent-ratio snapshot bridge body contract is incomplete';
  END IF;
  ...
  IF (
    ...
    WHERE version =
      '0077_runtime_authorized_management_follow_up_consent_ratio_snapshot_read'
  ) <> 1 THEN
    RAISE EXCEPTION
      'runtime follow-up consent-ratio snapshot bridge migration was not recorded once';
  END IF;
END
$check$;
