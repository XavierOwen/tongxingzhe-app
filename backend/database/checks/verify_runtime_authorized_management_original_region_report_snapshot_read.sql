...
DECLARE
  bridge regprocedure := to_regprocedure(
    'app_data.read_authorized_management_original_region_report_snapshot_v1(text,text,uuid,uuid)'
  );
...
BEGIN
  IF bridge IS NULL OR private_read IS NULL THEN
    RAISE EXCEPTION
      'runtime original-region snapshot bridge is incomplete';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'runtime original-region snapshot bridge owner/security is incorrect';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'runtime original-region snapshot bridge ACL is incorrect';
  END IF;
  ...
  FOREACH forbidden_role IN ARRAY ARRAY[
    ...
    IF has_function_privilege(forbidden_role, bridge, 'EXECUTE') THEN
      RAISE EXCEPTION
        'forbidden role can execute runtime original-region snapshot bridge: %',
        forbidden_role;
    END IF;
  END LOOP;
  ...
  THEN
    RAISE EXCEPTION
      'runtime received direct original-region identity or private access';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'runtime original-region snapshot bridge body contract is incomplete';
  END IF;
  ...
  IF (
    ...
    WHERE version =
      '0070_runtime_authorized_management_original_region_report_snapshot_read'
  ) <> 1 THEN
    RAISE EXCEPTION
      'runtime original-region snapshot bridge migration was not recorded once';
  END IF;
END
$check$;
