...
-- Structural and least-privilege checks for Slice 6CB.  The rollback fixture
-- proves the public declaration and lifecycle seams.  This check keeps the
-- current-city relation independent from the 6BE channel ledger and makes
...
BEGIN
  ...
  IF replacements_table_oid IS NULL THEN
    RAISE EXCEPTION
      'current-city snapshot replacement table is missing';
  END IF;
    ...
    OR NOT EXISTS (
      ...
      WHERE policy_row.polrelid = snapshots_table_oid
        ...
        AND pg_catalog.pg_get_expr(
          ...
          '((report_id = ''contact_sessions_by_current_city_two_periods''::text) '
          || 'AND (release_lineage_id = '
          || '''management-region-report:'
          || 'contact_sessions_by_current_city_two_periods''::text))'
    )
  THEN
    RAISE EXCEPTION
      'current-city snapshot replacement RLS scope is not exact';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'current-city replacement owner must be a closed NOLOGIN role: %',
      table_owner;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'current-city replacement table stores report values or source data';
  END IF;
  ...
  LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION
        'current-city replacement column contract is incomplete: %',
  ...
  IF (
    ...
    RAISE EXCEPTION
      'current-city replacement table must contain exactly seventeen audit columns';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'current-city replacement request primary key is incomplete';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION
      'current-city replacement snapshot uniqueness is incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'current-city replacement snapshot foreign keys are incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'current-city replacement table lacks the no-self-link invariant';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'current-city replacement history is not immutable';
  END IF;
  ...
  IF function_name IS NULL THEN
    RAISE EXCEPTION 'current-city replacement insert validator is missing';
  END IF;
  SELECT
  ...
  THEN
    RAISE EXCEPTION
      'current-city replacement validator security/provenance contract is incomplete';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE trigger_row.tgrelid = replacements_table_oid
      ...
      AND pg_get_triggerdef(trigger_row.oid) ILIKE
        '%validate_management_current_city_snapshot_replacement_v1%'
  ) THEN
    RAISE EXCEPTION
      'current-city replacement insert validator trigger is missing';
  END IF;
  ...
  IF trusted_function IS NULL THEN
    RAISE EXCEPTION
      'current-city trusted provenance helper is missing';
  END IF;
  SELECT
    pg_get_userbyid(procedure_row.proowner),
    procedure_row.prosecdef,
  ...
  IF function_owner IS DISTINCT FROM table_owner
    ...
    OR function_definition NOT ILIKE '%current_city_snapshot_replacement_provenance_v1%'
    ...
    OR function_definition NOT ILIKE '%source_change_sequence%'
    OR function_definition NOT ILIKE '%::bigint%'
  THEN
    RAISE EXCEPTION
      'current-city trusted provenance helper contract is incomplete';
  END IF;
  ...
  IF provenance_function IS NULL THEN
    RAISE EXCEPTION 'current-city replacement provenance seam is missing';
  END IF;
  SELECT
    pg_get_userbyid(procedure_row.proowner),
    procedure_row.prosecdef,
  ...
  IF function_owner IS DISTINCT FROM
    ...
    OR function_definition NOT ILIKE '%management_current_city_report_release_attempts%'
    OR function_definition NOT ILIKE '%attempt.result_status%'
    OR function_definition NOT ILIKE '%reason_codes%'
    OR function_definition NOT ILIKE '%target_context%'
    OR function_definition NOT ILIKE '%selection_sequence%'
    OR function_definition NOT ILIKE '%selection_source%'
    OR function_definition NOT ILIKE '%selection_evidence_at_utc%'
    OR function_definition NOT ILIKE '%tree_published_at_utc%'
  THEN
    RAISE EXCEPTION
      'current-city replacement provenance seam contract is incomplete';
  END IF;
  ...
  LOOP
    ...
    THEN
      RAISE EXCEPTION
        'current-city replacement internal function is directly executable: %',
        function_name;
    END IF;
  END LOOP;
  ...
  LOOP
    IF function_name IS NULL THEN
      RAISE EXCEPTION
        'current-city replacement public SQL seam is missing';
    END IF;
    ...
    THEN
      RAISE EXCEPTION
        'current-city replacement function owner/security is incorrect: %',
        function_name;
    END IF;
    ...
    IF function_name::text LIKE 'app_private.declare_%' THEN
      IF function_volatility <> 'v'
        ...
        OR function_definition NOT ILIKE
          '%management_current_city_snapshot_has_trusted_provenance_v1%'
        OR function_definition NOT ILIKE
          '%validate_management_current_city_report_document_v1%'
      THEN
        RAISE EXCEPTION
          'current-city replacement declaration lock/provenance contract is incomplete';
      END IF;
    ELSE
      ...
      THEN
        RAISE EXCEPTION
          'current-city replacement lifecycle query is not value-free';
      END IF;
    END IF;
  END LOOP;
  ...
  THEN
    RAISE EXCEPTION
      'lifecycle owner can bypass the current-city provenance bridge';
  END IF;
  ...
  THEN
    RAISE EXCEPTION
      'current-city replacement request claim contract is incomplete';
  END IF;

  -- The relation is private.  The current-city reader and release writer
  ...
  LOOP
    IF EXISTS (
      ...
      RAISE EXCEPTION
        'current-city replacement contract is directly executable by %',
        role_name;
    END IF;
  END LOOP;
  ...
  THEN
    RAISE EXCEPTION 'current-city replacement contract is open to PUBLIC';
  END IF;
  ...
  IF (
    ...
    WHERE version =
      '0080_management_current_city_report_snapshot_replacements'
  ) <> 1 THEN
    RAISE EXCEPTION
      'current-city snapshot replacement migration was not recorded once';
  END IF;
END
$check$;
