...
组织目录支持明确安排已知普通成员到项目：先核对、再提交，未知结果保留原请求重试。组织加入不自动建立项目关系，安排也不授管理或 PII 权限。
...
Drift 当前 v19 快照和 v5／v6／v8 至 v18→v19 升级检查：
...
跨操作测试另确认：批准申请只建立组织成员关系，明确安排才建立默认推广者项目关系；两步都不自动授管理权限。
...
- 推广对象离线 PII 只进入平台安全存储；Drift 仅保存不含 PII 的锁定状态；
