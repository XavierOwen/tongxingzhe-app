...
对象当次反应从当前有效 revision 的对象关联计数。页面从已有五档数量生成下中位等级，不再查询关联。未填关联仍只显示为覆盖缺口。

对象当次反应五档比例与分布共用这组五档数量。最近七日页面每档显示已填数量、分子／分母和百分比；例如 `2 / 9` 按 `2222` 基点计算，页面显示 `22.22%`。没有已填关联时显示 `0 / 0` 和空百分比，不显示 `0%`。
...
- 上传 ACK 后变为已同步；
- 启动时拉取其他设备的接触并刷新今日事实；
- “今日”显示场次、触达人数和待同步数；最近七日分析页另显示兴趣分布、各档比例、兴趣 `3–4` 与 `0` 子集比例、兴趣中位、对象当次反应分布、五档比例与中位、渠道、最近发生时间和同步覆盖。空分母显示对应空状态，覆盖说明读取实际缺失与候选内排除计数。
