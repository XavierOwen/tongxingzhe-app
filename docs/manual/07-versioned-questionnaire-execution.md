...
旧接触草稿始终按创建时的问卷版本打开、保存和提交。记录者可以继续使用旧版本，也可先预览跨版本答案分类，再明确新建当前版本草稿。这个过程不会静默改写原草稿。
...
旧草稿页面同时读取原问卷和项目当前问卷。用户点击“升级预览”后，App 先刷新当前草稿的自动保存，再把答案分为三类：

- 有已审计语义兼容决定，且在目标定义中仍通过题型、选项、范围和显示规则验证的答案，可以保留；
- 目标问卷中没有安全复制值的问题，必须重新查看和确认；
- 旧问卷中没有兼容证据或已不合法的答案，不能复制。

缺少兼容证据时系统失败关闭：它不会因问题 ID 相同就推测语义相同。路由在线取得当前兼容目录后，只把来源版本和目标版本同时属于同一稳定指标、并且由一条未撤销兼容事件直接连接的活动问题映射交给升级规划器。目录不可用、决定已撤销、问题不在同一指标或只有经第三个版本传递的关系时，预览会要求重新确认目标问题。

确认后，`ContactJournal` 使用来源草稿和目标版本生成稳定 ID，并在一个 SQLite transaction 中新建草稿、安全答案和私有同步 command。原草稿不在该 transaction 中被更新或删除。断网、重复点击或 ACK 丢失后重试，都会回到同一份新草稿。已正式提交但尚未同步的接触已不是草稿，它仍按提交时的精确问卷版本上传，不进入升级路径。
...
问卷管理页同时读取稳定指标、可比较的问题版本和追加式审计历史。管理员建立新指标或选择既有指标后，必须选择一个指标内基准问题和一个尚未加入指标的候选问题。确认按钮上方并排显示：

- 问题定义和题型；
- 选项 ID、顺序和显示文字；
- 固定的接触发生时间统计范围；
- 必填、未知、拒答和不适用等回答方式；
- 两个版本各自的可分析样本数，以及合并后的样本数。

页面不提供“按相似文字自动匹配”。操作者必须选择“兼容”或“不兼容”并填写理由。稳定指标只支持数量、分布和比例三种受控操作；短文本和长文本不进入候选目录。兼容问题必须保留可组合的数据形状，选择题还必须保留稳定 option ID。改变含义、时间范围、题型或无法映射的选项时，应记录“不兼容”。

每次 `GET /v1/questionnaire-metrics`、确认和撤销都会重新验证 access token、可信当前项目和 `manage_analysis_definitions` capability。Flutter 不提交用户、空间或项目来扩大范围。PostgreSQL 再复验活动个人空间所有者和项目，拒绝跨项目问题、文本问题、重复成员和不合法操作。

`questionnaire_metric_compatibility_events` 只追加，不更新。每条决定保存操作者、时间、理由、问题版本范围、当时的完整比较快照和样本影响。request ID 重放返回原事件；同 request ID 改写内容会冲突。数据库触发器阻止改写或删除指标与事件。两个会话同时确认同一候选或同时撤销同一事件时，指标级 transaction lock 和唯一约束只允许一个胜者。

`questionnaire_metric_members` 是动态分析读取的当前成员关系。兼容决定把候选问题加入稳定指标；不兼容决定只进入审计历史；撤销会追加新事件并删除候选的当前成员关系，因此后续动态分析重新分开。已生成报告不得回查这个可变成员表来解释历史结果，而应保存生成时事件携带的比较与影响快照。个人空间由所有者直接决定；当前尚无组织空间，因此没有默认双人审批。
...
[`0015_questionnaire_metric_compatibility.sql`](../../backend/database/migrations/0015_questionnaire_metric_compatibility.sql) 保存稳定指标、当前成员和不可变审计事件。runtime role 只能执行按可信上下文过滤的列表、决定和撤销函数，不能直接读写三张表。
...
[`questionnaire-draft-upgrade-contract-v1.json`](../../fixtures/questionnaire/questionnaire-draft-upgrade-contract-v1.json) 给出旧版本、目标版本、已审计兼容关系、原答案和三类预期结果。Flutter 与 Backend 读取同一文件并核对保留答案、待重新确认问题和不可复制来源。PostgreSQL `0014_questionnaire_draft_upgrades.sql` fixture 另外验证两份草稿共存、重试幂等、跨设备来源恢复、明确放弃原草稿和跨用户拒绝。

PostgreSQL `0015_questionnaire_metric_compatibility.sql` fixture 验证兼容、不兼容、幂等重放、撤销、当前成员重算和跨项目拒绝。独立 shell 检查在两个数据库会话中同时确认并同时撤销同一关系，核对每一步只有一个审计事件成为当前事实。
...
Drift migration 测试从保存的 v12 schema 写入 boolean 正式答案和草稿答案，再升级到 v19。独立的 v13 至 v18 升级用例还核对旧答案、问卷定义、设置和旧草稿保留；新的跳过原因、显示规则、管理工作副本、升级来源、对象关联、位置来源和当前关系快照不做猜测回填。

## 当前边界

当前实现完成管理草稿、受控编辑、模拟预览、版本差异、事务发布、并发与幂等保护、已发布版本授权读取、本地不可变缓存、八题型 UI、五状态、受限显示规则、隐藏答案确认与撤销、旧草稿明确升级、跨版本稳定指标、兼容与不兼容审计、撤销后动态分组、离线接触草稿、Flutter evaluator、Backend validator、PostgreSQL 复验和文本 warehouse 隔离。

当前分析页尚未提供项目问卷指标图表或报告导出界面。兼容目录和事件快照已经定义这两类消费者的安全输入：动态图表读取当前成员，固定报告保存生成时快照。后续实现不得让报表执行任意 SQL，也不得把文本答案带入默认结构化统计。
