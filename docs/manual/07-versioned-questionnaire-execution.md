...
缺少兼容证据时系统失败关闭：它不会因问题 ID 相同就推测语义相同。当前兼容决定的管理与撤销仍属后续能力；在没有可用决定时，预览会要求重新确认所有目标问题。
...
当前实现完成管理草稿、受控编辑、模拟预览、版本差异、事务发布、并发与幂等保护、已发布版本授权读取、本地不可变缓存、八题型 UI、五状态、受限显示规则、隐藏答案确认与撤销、旧草稿明确升级、离线接触草稿、Flutter evaluator、Backend validator、PostgreSQL 复验和文本 warehouse 隔离。

跨版本指标兼容决定的建立、审计和撤销尚未实现。在这些决定形成前，升级规划器不会把同 ID 当作兼容证据，也不会把旧版本答案自动解释成新版本答案。
