...
当前实现包含周目标、固定统计时区、周期起始日、版本历史、每日当地提醒、逐设备系统通知 opt-in、可选详细通知、本人 HTTP API、“今日”页卡片和离线副本。离线时可以保存一项计划修改；同步提醒时间仍保持只读。Web、Linux 和 Windows 也不能在 App 关闭后可靠运行每日重复调度。
...
[`DriftPersonalPlanningCache`](../../lib/plans/personal_planning_cache.dart) 使用 App 已有的 Drift／SQLite 设置表保存计划、提醒快照和一项待同步计划修改。它不是第二套授权或通用同步队列。每条 key 同时包含内部 app user、workspace 和项目 ID。一次请求开始时会捕获这个可信 scope；远端结果返回时若 scope 已改变，结果不会写入缓存。
...
只有设备已经取得计划快照或明确空值，断网保存才会建立本机草稿。草稿保存完整计划字段、原 revision、mutation ID 和首次排队 UTC 时间。没有可信缓存、scope 已改变或缓存损坏时，保存失败关闭。排队在 Drift transaction 内完成。一个 scope 最多保留一项草稿；并发保存不会覆盖第一项，使用者必须先同步或处理它，才能再次编辑。

联网载入计划后，gateway 用原 revision 和 mutation ID 自动重放草稿。服务端确认成功或识别为重复请求时，Drift 在一个 transaction 内写入权威快照并删除草稿。网络再次失败时保留原排队时间。授权失效会清除该 scope 的计划、提醒和草稿，并取消对应私人通知。

revision 冲突不会使用 last-write-wins。页面同时显示服务器计划和本机草稿。使用者可以保留服务器版本，也可以用服务器最新 revision 和新 mutation ID 明确提交本机修改。如果这次提交仍冲突，App 会重新读取服务器快照，下一次操作不会继续使用旧 revision。系统不做逐字段自动合并。服务端已有待生效版本时，本机草稿也不会覆盖它。

离线卡片显示“离线副本”和上次同步 UTC 时间。本机草稿只表达使用者的选择，不伪造服务端 progress 或生效边界。同步提醒时间保持只读；本设备的系统通知开关和通知内容偏好仍可修改，因为它们本来就是逐设备设置。若计划快照已经跨过 `cycle_until_utc`，界面会把它标成上一周期，不再称为本周计划。详细系统通知遇到过期周期或未同步计划修改时会降级为通用文案。草稿排队或删除后，计划卡片会立即通知提醒卡片重新调度，不等待 App 下次回到前台。
...
[`PrivateSessionDataGuard`](../../lib/app/private_session_data_guard.dart) 监听可信 App session。离开 workspace 或项目时，它只删除旧 scope 的待同步草稿，保留旧 scope 的只读缓存和已有提醒。登出、启动时未登录、账号切换或明确授权失败后，它会取消待发的私人提醒并清除全部计划缓存与本机草稿。单纯网络中断不会取消仍有效的提醒，也不会删除离线状态。当前项目的提醒或计划读取失去授权时，gateway 会清缓存，面板也会取消该项目的旧调度。App 长期关闭期间无法立即得知远端撤权；这是本地重复调度的残余限制。
...
语义层把“每日行动提醒”和“私人周计划”标成 heading。开关继续使用 Material 的单一合并语义节点。计划数、已记录数和差额各有一个节点，不重复朗读。载入与保存进度有具体名称，异步错误、冲突和表单校验错误使用 live region。离线说明包含“离线副本”、上次同步时间和可修改范围。待同步字段作为一个状态节点播报；重试、放弃和冲突选择保持独立按钮语义。
...
离线缓存和草稿使用测试进程中的 Drift／SQLite，不需要 Docker。相关 Widget 测试也不需要模拟器或真机。Docker 套件验证 Backend 的 PostgreSQL schema、权限、revision 冲突、mutation 幂等、周期计算、提醒版本和备份恢复。此次功能没有新增 PostgreSQL migration，但合并前仍运行 Docker 套件，确认现有服务端合同没有回归。两组测试不能互相代替。
...
通知实际触发和旅行时区的可观测性步骤见[提醒送达与旅行时区真机 Spike](../spikes/reminder-delivery-and-travel-time-zone.md)。安排成功、通知仍在通知中心和用户点击是三种不同证据，不能互相替代。
