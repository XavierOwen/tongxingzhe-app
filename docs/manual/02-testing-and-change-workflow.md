...
空库能按 v19 建成，只证明新安装可用，不能证明 v18 或更早版本的用户能安全升级。迁移测试先用保存的旧 schema 建库并写入 synthetic 数据，再运行正式 migration，核对原内容、默认值、索引、外键和新约束。
...
| v5／v6／v8 至 v18→v19 migration | 证明旧数据升级后保留内容并采用新约束 | 旧接触、草稿、答案和问卷定义保留；新规则、升级来源、对象关联和位置来源不猜测回填；新增当前关系快照表保持空白 | Red 后 Green，Drift migration 测试保留 |
