...
空库能按 v13 建成，只证明新安装可用，不能证明 v12 或更早版本的用户能安全升级。迁移测试先用保存的旧 schema 建库并写入 synthetic 数据，再运行正式 migration，核对原内容、默认值、索引、外键和新约束。
...
| v5／v6／v8／v9／v10／v11／v12→v13 migration | 证明旧数据升级后保留内容并采用新约束 | 旧接触、草稿和 boolean 答案保留；新定义表不伪造数据 | Red 后 Green，Drift migration 测试保留 |
| 问卷三端合同 | 防止 Flutter、Backend 和 PostgreSQL 对题型、状态或边界产生不同解释 | 八题型、五状态、权限、离线草稿和服务端复验通过 | Red 后 Green，共享 fixture 与三端测试保留 |
...
| 全部 Flutter tests | 检查已有合同未被破坏 | 全部通过 | 192 tests pass |
| Backend tests | 检查 JWT、可信上下文、项目、上传、拉取和错误分类 | TypeScript 检查与全部测试通过 | 55 tests pass |
...
| Markdown links | 防止说明书入口和链接失效 | 全部文档链接通过 | pass；128 files checked |
