...
空库能按 v12 建成，只证明新安装可用，不能证明 v11 或更早版本的用户能安全升级。迁移测试先用保存的旧 schema 建库并写入 synthetic 数据，再运行正式 migration，核对原内容、默认值、索引、外键和新约束。
...
| v5／v6／v8／v9／v10／v11→v12 migration | 证明旧数据升级后保留内容并采用新约束 | 旧接触和草稿保留，修正／作废、来源关联与冲突约束可用 | Red 后 Green，Drift migration 测试保留 |
...
| 全部 Flutter tests | 检查已有合同未被破坏 | 全部通过 | 181 tests pass |
| Backend tests | 检查 JWT、可信上下文、项目、上传、拉取和错误分类 | TypeScript 检查与全部测试通过 | 49 tests pass |
| PostgreSQL 16 重建与恢复 | 证明 migration、最小权限、区域、私有草稿、修订冲突、指标和备份可用 | 空库、checksum 重跑、fixture、`pg_dump`、`pg_restore` 全部通过 | pass |
