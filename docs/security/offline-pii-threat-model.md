...
撤权后重新开始且成功完成在线验权的请求可以建立快照。请求代次只保存在内存中，因为在途 Future 不跨进程重启；既有持久锁仍负责重启后的失败关闭。这不是跨进程同步机制。
...
7Z 的无下游关系成员退出先清除所选组织的本地快照，再发 HTTP。条件清除同步失效旧 subject 请求，在同一队列内解读唯一快照并 exact 比较 workspace；仅匹配时锁定并删除，不匹配的密文及锁保持原样。无法判断归属或删除失败时不发送退出请求。这个顺序避免退出已在服务端完成、成功响应却丢失时留下本地缓存；代价是服务端随后拒绝也不恢复已清缓存。

清除绑定原账号，HTTP 调用绑定一次不间断登录；等待 token、401 刷新或响应期间换账号、注销再登录都不能继续旧意图。结果不确定的请求只留在对话框内存，由用户重试或确认放弃。
当前正式组织对象 writer 与组织 SessionContext 尚未开放；未来开放时必须增加退出进行中对新组织资料请求的作用域门禁。这里的旧请求 fence 不等于持久 workspace 撤权，不声称清除了其他设备、系统备份或跨进程资料。
...
| 退出成功响应丢失后仍保留组织缓存 | 明确确认后先按 workspace 预清，失败不发 HTTP | 服务端拒绝时也不恢复缓存；其他设备与未来组织 writer 另需合同 |
