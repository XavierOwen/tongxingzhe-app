...
> **状态：legacy demo 历史资料。**本文描述的是早期原型，其中 `role_level`、管理者明细、统计阈值、认证和数据库等内容可能与后来确认的产品边界冲突。现代化实现以 [正式 Spec](./PRODUCT_SPEC.md)、[`CONTEXT.md`](../CONTEXT.md) 和已接受 [ADR](./adr/) 为准；请勿从本文直接创建新功能实现。
