...
/// interest 面板在一个明确管理项目范围内的读取阶段。
enum InterestReportPanelStage {
  /// 当前没有可读取的管理项目。
  inactive,
...
/// interest 面板的内存状态。
///
/// 这里故意只保存 interest gateway 的 typed 结果。它不保存个人项目、
/// channel 或 current-city report DTO，也不负责缓存或持久化。
final class InterestReportPanelState {
  InterestReportPanelState({
  ...
  factory InterestReportPanelState.initial(String? projectId) {
    final active = projectId != null;
    return InterestReportPanelState(
      projectId: projectId,
      stage: active
          ? InterestReportPanelStage.loadingDirectory
          : InterestReportPanelStage.inactive,
    );
  }

  factory InterestReportPanelState.inactive() =>
      InterestReportPanelState.initial(null);

  final String? projectId;
  final InterestReportPanelStage stage;
  final InterestReportSnapshotDirectory? directory;
  final InterestReportSnapshotSummary? selectedSummary;
  final InterestReportSnapshot? snapshot;
  final InterestReportFailureCode? failureCode;

  bool get isLoading =>
      stage == InterestReportPanelStage.loadingDirectory ||
      stage == InterestReportPanelStage.loadingSnapshot;

  bool get isEmpty =>
      stage == InterestReportPanelStage.directory &&
      directory != null &&
      directory!.snapshots.isEmpty;
}

/// interest 管理报告的独立读取状态机。
///
/// [projectId] 必须由上层已经解析并重新授权的 management context 提供；
/// `null` 表示面板停用。每次异步读取都绑定当前 generation，项目切换、返回
/// 目录和 dispose 会使旧响应失效。
final class InterestReportPanelViewModel extends ChangeNotifier {
  InterestReportPanelViewModel({
    required this._gateway,
    required String? projectId,
  }) : _projectId = projectId,
       _state = InterestReportPanelState.initial(projectId);

  final InterestReportGateway _gateway;
  String? _projectId;
  InterestReportPanelState _state;
  _InterestReportRetryOperation? _retryOperation;
  var _generation = 0;
  var _disposed = false;

  InterestReportPanelState get state => _state;
  ...
  Future<void> loadDirectory() async {
    ...
    if (projectId == null) {
      _invalidateAndSet(InterestReportPanelState.inactive());
      return;
    }
    ...
    InterestReportResult<InterestReportSnapshotDirectory> result;
    try {
      result = await _gateway.listSnapshots(projectId);
    } catch (_) {
      result = const InterestReportRejected(
    ...
    switch (result) {
      case InterestReportSuccess(:final value):
      ...
      case InterestReportRejected(:final code):
        _setState(
          InterestReportPanelState(
            projectId: projectId,
  ...
  Future<void> updateProject(String? projectId) async {
    ...
    _retryOperation = null;
    if (projectId == null) {
      _setState(InterestReportPanelState.inactive());
      return;
    }
  ...
  /// 只允许打开当前目录中用户明确选中的摘要。
  Future<void> openSnapshot(InterestReportSnapshotSummary summary) async {
    final projectId = _projectId;
    ...
    InterestReportResult<InterestReportSnapshot> result;
    try {
  ...
  void _invalidateAndSet(InterestReportPanelState state) {
    ++_generation;
    _retryOperation = null;
    _setState(state);
  }

  void _setState(InterestReportPanelState state) {
    if (_disposed) return;
    _state = state;
    notifyListeners();
  }
  ...
  void dispose() {
    ...
    ++_generation;
    _retryOperation = null;
    _state = InterestReportPanelState.inactive();
    super.dispose();
  }
}

sealed class _InterestReportRetryOperation {
  const _InterestReportRetryOperation();
}

final class _RetryLoadDirectory extends _InterestReportRetryOperation {
  const _RetryLoadDirectory(this.projectId);

  final String projectId;
}

final class _RetryReadSnapshot extends _InterestReportRetryOperation {
