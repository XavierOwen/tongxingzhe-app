...
# Two independent sessions prove that the interest directory and capability
# revocation use the same authorization lock order. The directory intentionally
# has no snapshots here: the test is about linearization and value-free audit,
# not report fixture construction.
...
wait_for_ready() {
  ...
  echo 'interest directory concurrency session did not hold its ready lock.' >&2
...
"${psql_base[@]}" --command="
  ...
  INSERT INTO app_data.app_users (app_user_id, status) VALUES
    ('a6d10000-0000-4000-8000-000000000001'::uuid, 'active'),
    ('a6d10000-0000-4000-8000-000000000002'::uuid, 'active');

  INSERT INTO app_data.external_identities (
    external_identity_id, issuer, subject, app_user_id
  ) VALUES
    (
      'a6de0000-0000-4000-8000-000000000001'::uuid,
      'https://interest-directory-concurrency.synthetic/auth/v1',
      'directory-first',
      'a6d10000-0000-4000-8000-000000000001'::uuid
    ),
    (
      'a6de0000-0000-4000-8000-000000000002'::uuid,
      'https://interest-directory-concurrency.synthetic/auth/v1',
      'revocation-first',
      'a6d10000-0000-4000-8000-000000000002'::uuid
    );

  INSERT INTO app_data.workspaces (
    workspace_id, workspace_kind, display_name
  ) VALUES (
    'a6d20000-0000-4000-8000-000000000001'::uuid,
    'organization',
    'Interest snapshot directory concurrency organization'
  );

  INSERT INTO app_data.projects (
    project_id, workspace_id, display_name
  ) VALUES (
    'a6d30000-0000-4000-8000-000000000001'::uuid,
    'a6d20000-0000-4000-8000-000000000001'::uuid,
    'Interest snapshot directory concurrency project'
  );
  ...
  INSERT INTO app_data.organization_memberships (
    ...
    (
      'a6d40000-0000-4000-8000-000000000002'::uuid,
      'a6d20000-0000-4000-8000-000000000001'::uuid,
      'a6d10000-0000-4000-8000-000000000002'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.organization_owner_assignments (
    ...
    'a6da0000-0000-4000-8000-000000000001'::uuid,
    'a6d40000-0000-4000-8000-000000000001'::uuid,
  ...
  INSERT INTO app_data.project_memberships (
    ...
    (
      'a6d50000-0000-4000-8000-000000000001'::uuid,
      'a6d40000-0000-4000-8000-000000000001'::uuid,
      'a6d30000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      'a6d50000-0000-4000-8000-000000000002'::uuid,
      'a6d40000-0000-4000-8000-000000000002'::uuid,
      'a6d30000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.management_report_capability_grants (
    ...
    (
      'a6d60000-0000-4000-8000-000000000001'::uuid,
      'a6d50000-0000-4000-8000-000000000001'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    ),
    (
      'a6d60000-0000-4000-8000-000000000002'::uuid,
      'a6d50000-0000-4000-8000-000000000002'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    );
...
directory_first_ready='interest-directory-ready:directory-first'
...
"${psql_base[@]}" --command="
  ...
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:a6d20000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:a6d30000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:a6d30000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000001:view_anonymous_analytics',
    0
  ));
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = 'a6d60000-0000-4000-8000-000000000001'::uuid;
  COMMIT;
...
revocation_first_ready='interest-directory-ready:revocation-first'

"${psql_base[@]}" --command="
  SET statement_timeout = '20s';
  BEGIN;
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:a6d20000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:a6d30000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:a6d30000-0000-4000-8000-000000000001:a6d10000-0000-4000-8000-000000000002:view_anonymous_analytics',
    0
  ));
  ...
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = 'a6d60000-0000-4000-8000-000000000002'::uuid;
  COMMIT;
...
directory_second_count="$("${psql_base[@]}" --tuples-only --no-align --command="
  ...
  WHERE requested_by_app_user_id = 'a6d10000-0000-4000-8000-000000000002'::uuid;
" | tr -d '[:space:]')"
...
echo 'interest snapshot directory concurrency check passed: read and revocation linearized on shared authorization locks.'
