...
# Two independent sessions prove that the current-city directory and capability
...
wait_for_ready() {
  ...
  echo 'current-city directory concurrency session did not hold its ready lock.' >&2
...
"${psql_base[@]}" --command="
  ...
  INSERT INTO app_data.external_identities (
    ...
    (
      'a6ce0000-0000-4000-8000-000000000001'::uuid,
      'https://current-city-directory-concurrency.synthetic/auth/v1',
      'directory-first',
      'a6c10000-0000-4000-8000-000000000001'::uuid
    ),
    (
      'a6ce0000-0000-4000-8000-000000000002'::uuid,
      'https://current-city-directory-concurrency.synthetic/auth/v1',
      'revocation-first',
      'a6c10000-0000-4000-8000-000000000002'::uuid
    );

  INSERT INTO app_data.workspaces (
    workspace_id, workspace_kind, display_name
  ) VALUES (
    'a6c20000-0000-4000-8000-000000000001'::uuid,
    'organization',
    'Current-city directory concurrency organization'
  );

  INSERT INTO app_data.projects (
    project_id, workspace_id, display_name
  ) VALUES (
    'a6c30000-0000-4000-8000-000000000001'::uuid,
    'a6c20000-0000-4000-8000-000000000001'::uuid,
    'Current-city directory concurrency project'
  );
  ...
  INSERT INTO app_data.organization_memberships (
    ...
    (
      'a6c40000-0000-4000-8000-000000000002'::uuid,
      'a6c20000-0000-4000-8000-000000000001'::uuid,
      'a6c10000-0000-4000-8000-000000000002'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.project_memberships (
    ...
    (
      'a6c50000-0000-4000-8000-000000000001'::uuid,
      'a6c40000-0000-4000-8000-000000000001'::uuid,
      'a6c30000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      'a6c50000-0000-4000-8000-000000000002'::uuid,
      'a6c40000-0000-4000-8000-000000000002'::uuid,
      'a6c30000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.management_report_capability_grants (
    ...
    (
      'a6c60000-0000-4000-8000-000000000001'::uuid,
      'a6c50000-0000-4000-8000-000000000001'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    ),
    (
      'a6c60000-0000-4000-8000-000000000002'::uuid,
      'a6c50000-0000-4000-8000-000000000002'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    );
...
directory_first_ready='current-city-directory-ready:directory-first'
...
"${psql_base[@]}" --command="
  ...
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:a6c20000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:a6c30000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:a6c30000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000001:view_anonymous_analytics',
    0
  ));
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = 'a6c60000-0000-4000-8000-000000000001'::uuid;
  COMMIT;
...
directory_first_state="$("${psql_base[@]}" --tuples-only --no-align --field-separator='|' --command="
  ...
  FROM app_private.management_current_city_report_snapshot_directory_access_events AS event
...
revocation_first_ready='current-city-directory-ready:revocation-first'

"${psql_base[@]}" --command="
  SET statement_timeout = '20s';
  BEGIN;
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:a6c20000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:a6c30000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:a6c30000-0000-4000-8000-000000000001:a6c10000-0000-4000-8000-000000000002:view_anonymous_analytics',
    0
  ));
  ...
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = 'a6c60000-0000-4000-8000-000000000002'::uuid;
  COMMIT;
...
directory_second_count="$("${psql_base[@]}" --tuples-only --no-align --command="
  ...
  WHERE requested_by_app_user_id = 'a6c10000-0000-4000-8000-000000000002'::uuid;
" | tr -d '[:space:]')"
...
echo 'current-city snapshot directory concurrency check passed: read and revocation linearized on shared authorization locks.'
