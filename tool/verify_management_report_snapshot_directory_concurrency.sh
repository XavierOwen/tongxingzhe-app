...
# Independent sessions prove that the bounded snapshot directory and capability
# revocation linearize on the shared Slice 6I authorization locks. The script
# calls the production runtime bridge so identity mapping and private audit are
# committed in the same transaction as the directory result.
...
"${psql_base[@]}" --command="
  ...
  INSERT INTO app_data.app_users (app_user_id, status) VALUES
    ('7e100000-0000-4000-8000-000000000001'::uuid, 'active'),
    ('7e100000-0000-4000-8000-000000000002'::uuid, 'active');

  INSERT INTO app_data.external_identities (
    external_identity_id, issuer, subject, app_user_id
  ) VALUES
    (
      '7ee00000-0000-4000-8000-000000000001'::uuid,
      'https://snapshot-directory-concurrency.synthetic/auth/v1',
      'directory-first',
      '7e100000-0000-4000-8000-000000000001'::uuid
    ),
    (
      '7ee00000-0000-4000-8000-000000000002'::uuid,
      'https://snapshot-directory-concurrency.synthetic/auth/v1',
      'revocation-first',
      '7e100000-0000-4000-8000-000000000002'::uuid
    );

  INSERT INTO app_data.workspaces (
    workspace_id, workspace_kind, display_name
  ) VALUES (
    '7e200000-0000-4000-8000-000000000001'::uuid,
    'organization',
    'Concurrent snapshot directory organization'
  );

  INSERT INTO app_data.projects (
    project_id, workspace_id, display_name
  ) VALUES (
    '7e300000-0000-4000-8000-000000000001'::uuid,
    '7e200000-0000-4000-8000-000000000001'::uuid,
    'Concurrent snapshot directory project'
  );
  ...
  INSERT INTO app_data.organization_memberships (
    ...
    (
      '7e400000-0000-4000-8000-000000000001'::uuid,
      '7e200000-0000-4000-8000-000000000001'::uuid,
      '7e100000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      '7e400000-0000-4000-8000-000000000002'::uuid,
      '7e200000-0000-4000-8000-000000000001'::uuid,
      '7e100000-0000-4000-8000-000000000002'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.organization_owner_assignments (
    ...
    '7e700000-0000-4000-8000-000000000001'::uuid,
    '7e400000-0000-4000-8000-000000000001'::uuid,
  ...
  INSERT INTO app_data.project_memberships (
    ...
    (
      '7e500000-0000-4000-8000-000000000001'::uuid,
      '7e400000-0000-4000-8000-000000000001'::uuid,
      '7e300000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      '7e500000-0000-4000-8000-000000000002'::uuid,
      '7e400000-0000-4000-8000-000000000002'::uuid,
      '7e300000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.management_report_capability_grants (
    ...
    (
      '7e600000-0000-4000-8000-000000000001'::uuid,
      '7e500000-0000-4000-8000-000000000001'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    ),
    (
      '7e600000-0000-4000-8000-000000000002'::uuid,
      '7e500000-0000-4000-8000-000000000002'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    );
...
directory_first_ready='snapshot-directory-ready:directory-first'
...
wait_for_ready \
  "${directory_first_ready}" \
  "${directory_first_pid}" \
  "${directory_first_output}"
...
"${psql_base[@]}" --command="
  ...
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:7e200000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:7e300000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:7e300000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000001:view_anonymous_analytics',
    0
  ));
...
if [[ "${directory_first_status}" -ne 0 || "${revocation_second_status}" -ne 0 ]]; then
  echo "目录先行撤权并发错误：directory=${directory_first_status}, revocation=${revocation_second_status}" >&2
  sed -n '1,160p' "${directory_first_output}" >&2
  sed -n '1,160p' "${revocation_second_output}" >&2
  exit 1
fi

directory_first_state="$("${psql_base[@]}" \
  --tuples-only --no-align --field-separator='|' --command="
    SELECT
      count(*),
      min(event.result_status),
      min(event.returned_snapshot_count),
      bool_and(capability_grant.inactive_from_utc IS NOT NULL)
    FROM app_private.management_report_snapshot_directory_access_events AS event
...
if [[ "${directory_first_state}" != '1|completed|0|t' ]]; then
  echo "目录先行没有在撤权前提交空目录及审计：${directory_first_state}" >&2
  exit 1
fi
...
revocation_first_ready='snapshot-directory-ready:revocation-first'

"${psql_base[@]}" --command="
  SET statement_timeout = '20s';
  BEGIN;
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:7e200000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:7e300000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:7e300000-0000-4000-8000-000000000001:7e100000-0000-4000-8000-000000000002:view_anonymous_analytics',
    0
  ));
...
if [[ "${revocation_first_status}" -ne 0 || "${directory_second_status}" -eq 0 ]]; then
  echo "撤权先行目录并发错误：revocation=${revocation_first_status}, directory=${directory_second_status}" >&2
...
if ! grep -q 'management report authorization forbidden' \
  "${directory_second_output}"; then
  echo '等待撤权的快照目录没有按授权失败关闭。' >&2
  sed -n '1,160p' "${directory_second_output}" >&2
  exit 1
fi

directory_second_count="$("${psql_base[@]}" \
...
if [[ "${directory_second_count}" -ne 0 ]]; then
  echo "撤权先行的目录读取留下了访问审计：${directory_second_count}" >&2
  exit 1
fi

echo '管理报告快照目录并发检查通过：目录读取与撤权按共享授权锁顺序完成。'
