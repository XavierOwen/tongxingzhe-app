...
# Two independent sessions prove that the original-region directory and
# capability revocation use the same authorization lock order. The directory
# has no snapshots here: this test proves linearization and value-free audit.
...
wait_for_ready() {
  ...
  echo 'original-region directory concurrency session did not hold its ready lock.' >&2
...
"${psql_base[@]}" --command="
  ...
  INSERT INTO app_data.app_users (app_user_id, status) VALUES
    ('6b7c1000-0000-4000-8000-000000000001'::uuid, 'active'),
    ('6b7c1000-0000-4000-8000-000000000002'::uuid, 'active');

  INSERT INTO app_data.external_identities (
    external_identity_id, issuer, subject, app_user_id
  ) VALUES
    (
      '6b7ce000-0000-4000-8000-000000000001'::uuid,
      'https://original-directory-concurrency.synthetic/auth/v1',
      'directory-first',
      '6b7c1000-0000-4000-8000-000000000001'::uuid
    ),
    (
      '6b7ce000-0000-4000-8000-000000000002'::uuid,
      'https://original-directory-concurrency.synthetic/auth/v1',
      'revocation-first',
      '6b7c1000-0000-4000-8000-000000000002'::uuid
    );

  INSERT INTO app_data.workspaces (
    workspace_id, workspace_kind, display_name
  ) VALUES (
    '6b7c2000-0000-4000-8000-000000000001'::uuid,
    'organization',
    'Original-region directory concurrency organization'
  );

  INSERT INTO app_data.projects (
    project_id, workspace_id, display_name
  ) VALUES (
    '6b7c3000-0000-4000-8000-000000000001'::uuid,
    '6b7c2000-0000-4000-8000-000000000001'::uuid,
    'Original-region directory concurrency project'
  );
  ...
  INSERT INTO app_data.organization_memberships (
    ...
    (
      '6b7c4000-0000-4000-8000-000000000001'::uuid,
      '6b7c2000-0000-4000-8000-000000000001'::uuid,
      '6b7c1000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      '6b7c4000-0000-4000-8000-000000000002'::uuid,
      '6b7c2000-0000-4000-8000-000000000001'::uuid,
      '6b7c1000-0000-4000-8000-000000000002'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.organization_owner_assignments (
    ...
    '6b7c7000-0000-4000-8000-000000000001'::uuid,
    '6b7c4000-0000-4000-8000-000000000001'::uuid,
  ...
  INSERT INTO app_data.project_memberships (
    ...
    (
      '6b7c5000-0000-4000-8000-000000000001'::uuid,
      '6b7c4000-0000-4000-8000-000000000001'::uuid,
      '6b7c3000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    ),
    (
      '6b7c5000-0000-4000-8000-000000000002'::uuid,
      '6b7c4000-0000-4000-8000-000000000002'::uuid,
      '6b7c3000-0000-4000-8000-000000000001'::uuid,
      clock_timestamp() - interval '1 day'
    );
  ...
  INSERT INTO app_data.management_report_capability_grants (
    ...
    (
      '6b7c6000-0000-4000-8000-000000000001'::uuid,
      '6b7c5000-0000-4000-8000-000000000001'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    ),
    (
      '6b7c6000-0000-4000-8000-000000000002'::uuid,
      '6b7c5000-0000-4000-8000-000000000002'::uuid,
      'view_anonymous_analytics',
      clock_timestamp() - interval '1 day'
    );
...
directory_first_ready='original-directory-ready:directory-first'
...
"${psql_base[@]}" --command="
  ...
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:6b7c2000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:6b7c3000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000001',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:6b7c3000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000001:view_anonymous_analytics',
    0
  ));
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = '6b7c6000-0000-4000-8000-000000000001'::uuid;
  COMMIT;
...
directory_first_state="$("${psql_base[@]}" --tuples-only --no-align --field-separator='|' --command="
  ...
  FROM app_private.management_original_region_snapshot_directory_access_events AS event
...
if [[ "${directory_first_state}" != '1|completed|0|t' ]]; then
  echo "directory-first did not commit its audited read before revocation: ${directory_first_state}" >&2
  exit 1
fi
...
revocation_first_ready='original-directory-ready:revocation-first'

"${psql_base[@]}" --command="
  SET statement_timeout = '20s';
  BEGIN;
  SELECT pg_advisory_xact_lock(hashtextextended(
    'organization-membership:6b7c2000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'project-membership:6b7c3000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000002',
    0
  ));
  SELECT pg_advisory_xact_lock(hashtextextended(
    'management-report-capability:6b7c3000-0000-4000-8000-000000000001:6b7c1000-0000-4000-8000-000000000002:view_anonymous_analytics',
    0
  ));
  ...
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
  WHERE capability_grant_id = '6b7c6000-0000-4000-8000-000000000002'::uuid;
  COMMIT;
...
directory_second_count="$("${psql_base[@]}" --tuples-only --no-align --command="
  ...
  WHERE requested_by_app_user_id = '6b7c1000-0000-4000-8000-000000000002'::uuid;
" | tr -d '[:space:]')"
...
echo 'original-region snapshot directory concurrency check passed: read and revocation linearized on shared authorization locks.'
