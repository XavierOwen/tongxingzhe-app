...
project_id='82c30000-0000-4000-8000-000000000001'
workspace_id='82c20000-0000-4000-8000-000000000001'
user_one='82c10000-0000-4000-8000-000000000001'
user_two='82c10000-0000-4000-8000-000000000002'
capability_one='82c60000-0000-4000-8000-000000000001'
capability_two='82c60000-0000-4000-8000-000000000002'
report_id='contact_sessions_by_interest_level_two_periods'
release_lineage="management-interest-report:${report_id}"
lineage_lock="management-interest-snapshot-replacement-lineage:${project_id}:${release_lineage}"
...
echo '建立 6CD concurrency interest snapshots。'
run_psql --quiet <<'SQL'
BEGIN;

INSERT INTO app_data.app_users (app_user_id, status) VALUES
  ('82c10000-0000-4000-8000-000000000001'::uuid, 'active'),
  ('82c10000-0000-4000-8000-000000000002'::uuid, 'active');

INSERT INTO app_data.workspaces (
  workspace_id, workspace_kind, display_name
) VALUES (
  '82c20000-0000-4000-8000-000000000001'::uuid,
  'organization', '6CD concurrency workspace'
);

INSERT INTO app_data.projects (project_id, workspace_id, display_name)
VALUES (
  '82c30000-0000-4000-8000-000000000001'::uuid,
  '82c20000-0000-4000-8000-000000000001'::uuid,
  '6CD concurrency project'
);
...
INSERT INTO app_data.organization_memberships (
  ...
  (
    ...
    '82c20000-0000-4000-8000-000000000001'::uuid,
    '82c10000-0000-4000-8000-000000000001'::uuid,
    '2025-01-01T00:00:00Z', NULL
  ),
  (
    '82c40000-0000-4000-8000-000000000002'::uuid,
    '82c20000-0000-4000-8000-000000000001'::uuid,
    '82c10000-0000-4000-8000-000000000002'::uuid,
    '2025-01-01T00:00:00Z', NULL
  );
...
INSERT INTO app_data.project_memberships (
  ...
  (
    '82c50000-0000-4000-8000-000000000001'::uuid,
    '82c40000-0000-4000-8000-000000000001'::uuid,
    '82c30000-0000-4000-8000-000000000001'::uuid,
    '2025-01-01T00:00:00Z', NULL
  ),
  (
    '82c50000-0000-4000-8000-000000000002'::uuid,
    '82c40000-0000-4000-8000-000000000002'::uuid,
    '82c30000-0000-4000-8000-000000000001'::uuid,
    '2025-01-01T00:00:00Z', NULL
  );
...
INSERT INTO app_data.management_report_capability_grants (
  ...
  (
    '82c60000-0000-4000-8000-000000000001'::uuid,
    '82c50000-0000-4000-8000-000000000001'::uuid,
    'release_management_reports', '2025-01-01T00:00:00Z', NULL
  ),
  (
    '82c60000-0000-4000-8000-000000000002'::uuid,
    '82c50000-0000-4000-8000-000000000002'::uuid,
    'release_management_reports', '2025-01-01T00:00:00Z', NULL
  );

SELECT app_private.configure_project_reporting_time_zone_v1(
  '82c70000-0000-4000-8000-000000000001'::uuid,
  '82c10000-0000-4000-8000-000000000001'::uuid,
  '82c30000-0000-4000-8000-000000000001'::uuid,
  0, 'UTC', '2025-01-01T00:00:00Z'
);
...
while IFS= read -r snapshot_id; do
  ...
  run_psql --tuples-only --no-align --command="
    ...
    WHERE project_id = '${project_id}'::uuid
      AND release_request_id::text LIKE '82c80000-0000-4000-8000-%'
    ORDER BY release_request_id;
  "
)
if [[ "${#snapshot_ids[@]}" -ne 9 ]]; then
  echo "6CD concurrency 需要 9 份 approved snapshot，实际为 ${#snapshot_ids[@]}。" >&2
  exit 1
fi
...
race_ready_lock='6cd-concurrency-race-ready'
...
run_psql --quiet --command="
  ...
  SELECT app_private.declare_management_interest_snapshot_replacement_v1(
    '82c90000-0000-4000-8000-000000000001'::uuid,
...
run_psql --quiet --command="
  SELECT app_private.declare_management_interest_snapshot_replacement_v1(
    '82c90000-0000-4000-8000-000000000002'::uuid,
...
if [[ "${race_first_status}" -ne 0 || "${race_second_status}" -eq 0 ]]; then
  echo "6CD competing replacements 结果错误：first=${race_first_status}, second=${race_second_status}" >&2
...
if ! grep -Eqi 'active head|stale|already.*replacement' "${race_second_output}"; then
  echo '第二个 6CD replacement 没有以 stale active head 失败关闭。' >&2
  sed -n '1,160p' "${race_second_output}" >&2
  exit 1
fi

echo '验证 replacement-first 后撤权按锁顺序提交。'
replacement_ready_lock='6cd-concurrency-replacement-first-ready'
...
run_psql --quiet --command="
  ...
  SELECT pg_advisory_lock(hashtextextended('${lineage_lock}', 0));
  SELECT app_private.declare_management_interest_snapshot_replacement_v1(
    '82c90000-0000-4000-8000-000000000003'::uuid,
    '${user_one}'::uuid, '${project_id}'::uuid,
    '${fourth_snapshot}'::uuid, '${fifth_snapshot}'::uuid,
    'contact_revision'
  );
  SELECT pg_advisory_lock(
    hashtextextended('${replacement_ready_lock}', 0)
  );
  SELECT pg_sleep(3);
  COMMIT;
  SELECT pg_advisory_unlock(hashtextextended('${lineage_lock}', 0));
...
run_psql --quiet --command="
  ...
  SELECT pg_advisory_xact_lock(
    hashtextextended('${organization_lock_one}', 0)
  );
  SELECT pg_advisory_xact_lock(
    hashtextextended('${project_lock_one}', 0)
  );
  SELECT pg_advisory_xact_lock(
    hashtextextended('${capability_lock_one}', 0)
  );
...
if [[ "${replacement_status}" -ne 0 || "${replacement_revoke_status}" -ne 0 ]]; then
  echo "6CD replacement-first 锁顺序错误：replacement=${replacement_status}, revoke=${replacement_revoke_status}" >&2
...
revoke_ready_lock='6cd-concurrency-revoke-first-ready'
revoke_output="${temporary_directory}/revoke-first.out"
revoke_replacement_output="${temporary_directory}/revoke-replacement.out"
run_psql --quiet --command="
  BEGIN;
  SELECT pg_advisory_xact_lock(
    hashtextextended('${organization_lock_two}', 0)
  );
  SELECT pg_advisory_xact_lock(
    hashtextextended('${project_lock_two}', 0)
  );
  SELECT pg_advisory_xact_lock(
    hashtextextended('${capability_lock_two}', 0)
  );
  UPDATE app_data.management_report_capability_grants
  SET inactive_from_utc = clock_timestamp()
...
run_psql --quiet --command="
  SELECT app_private.declare_management_interest_snapshot_replacement_v1(
    '82c90000-0000-4000-8000-000000000004'::uuid,
    '${user_two}'::uuid, '${project_id}'::uuid,
...
if [[ "${revoke_status}" -ne 0 || "${revoke_replacement_status}" -eq 0 ]]; then
  echo "6CD revoke-first 锁顺序错误：revoke=${revoke_status}, replacement=${revoke_replacement_status}" >&2
...
if ! grep -Eqi 'authorization forbidden|authorization.*denied|capability' \
  ...
  echo '撤权先提交后 6CD replacement 没有失败关闭。' >&2
  sed -n '1,160p' "${revoke_replacement_output}" >&2
  exit 1
fi

echo '恢复授权并验证 release-first request family claim。'
run_psql --quiet --command="
  INSERT INTO app_data.management_report_capability_grants (
    capability_grant_id, project_membership_id, capability_id,
    active_from_utc, inactive_from_utc
    ...
    (
      '82c60000-0000-4000-8000-000000000003'::uuid,
      '82c50000-0000-4000-8000-000000000001'::uuid,
      'release_management_reports', clock_timestamp(), NULL
    ),
    (
      '82c60000-0000-4000-8000-000000000004'::uuid,
      '82c50000-0000-4000-8000-000000000002'::uuid,
      'release_management_reports', clock_timestamp(), NULL
    );
"

release_claim_request='82cd0000-0000-4000-8000-000000000001'
release_claim_lock="management-report-release-request:${release_claim_request}"
release_claim_ready='6cd-concurrency-release-claim-ready'
...
if [[ "${release_claim_status}" -ne 0 \
  ...
  echo "6CD release-first family claim 错误：release=${release_claim_status}, replacement=${release_claim_replacement_status}" >&2
...
replacement_claim_request='82cd0000-0000-4000-8000-000000000002'
replacement_claim_lock="management-report-release-request:${replacement_claim_request}"
replacement_claim_ready='6cd-concurrency-replacement-claim-ready'
...
run_psql --quiet --command="
  ...
  SELECT pg_advisory_lock(
    hashtextextended('${replacement_claim_ready}', 0)
  );
  SELECT pg_sleep(3);
  COMMIT;
  SELECT pg_advisory_unlock(
...
if [[ "${replacement_claim_status}" -ne 0 \
  ...
  echo "6CD replacement-first family claim 错误：replacement=${replacement_claim_status}, release=${replacement_claim_release_status}" >&2
...
if [[ "${final_state}" != '1|1|0|1' ]]; then
  echo "6CD concurrency 最终 lineage 状态错误：${final_state}" >&2
  exit 1
fi

echo '6CD interest replacement concurrency check passed: same-head, replacement/revocation and release/replacement request-claim ordering hold.'
